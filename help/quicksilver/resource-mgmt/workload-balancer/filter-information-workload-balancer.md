...
>[!IMPORTANT]
>
>为了高效地查找工作项并专注于您管理的用户或项，我们强烈建议您在工作负载均衡器中使用过滤器。 这允许您在开始管理资源的工作分配之前显示正确的信息。
>
>当您在保存并应用新过滤器后导航离开工作负载均衡器时，即使您注销并重新登录，过滤器仍会应用。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>当选择<b>此项目的工作项</b>系统默认筛选器时，分配给选定项目上至少一个工作项以及该项目上其工作项的用户。 </p>

<p>取消选择<b>此项目的工作项</b>系统默认筛选器时，项目的“已分配工作”区域显示分配给选定项目上至少一个项的用户的所有工作项。  <br><br>默认情况下取消选择此筛选器。</p>

<p> <b>注意</b>：您可以在项目的工作负载均衡器中启用<b>显示所有用户</b>选项以显示系统中的所有用户。 有关详细信息，请参阅<a href="../workload-balancer/navigate-the-workload-balancer.md" class="MCXref xref">导航工作负载均衡器</a></p>
...
1. 在&#x200B;**未分配的工作**&#x200B;或&#x200B;**已分配的工作**&#x200B;区域的右上角，单击&#x200B;**筛选器**&#x200B;图标![筛选器图标](assets/filter-icon.png)。 **筛选器**&#x200B;框显示在右侧，您为其创建筛选器的区域的名称显示在标题中。

   ![筛选器列表](assets/filters-list.png)

1. （可选且有条件）如果您访问&#x200B;**资源**&#x200B;区域的工作负载均衡器，则预定义的默认筛选器可能已应用于&#x200B;**已分配的工作**&#x200B;区域。 您可以编辑并保存默认筛选器的副本。

   >[!TIP]
   >
   >默认筛选器显示属于您的任何团队的用户及其工作项。 您可以编辑此筛选器的副本。

   如果从项目访问[!UICONTROL 工作负载均衡器]，则可能已应用&#x200B;**此项目的工作项**&#x200B;过滤器。 此项仅显示分配给此项目中的用户的工作项。 您可以复制并保存此筛选器的副本。
...
1. 要创建过滤器，请执行以下操作：
   ...
   1. 在第一个下拉菜单中选择字段名称，或单击下拉菜单底部的&#x200B;**浏览字段**&#x200B;以键入默认不显示的字段名称。
   ...
   1. （视情况而定）如果您单击&#x200B;**浏览字段**，请在&#x200B;**搜索**&#x200B;字段中键入字段的名称，并在该字段显示在列表中时将其选定。
      ...
      ![搜索字段](assets/new-filters-search-for-a-field.png)
   ...
   1. 为要过滤的字段选择或键入值。
      ...
      >[!NOTE]
      >
      > 要显示特定项目组合中的工作对象，可以应用以下过滤器： Portfolio ID等于&lt; PORTFOLIO NAME >。
      >
      >![Portfolio名称筛选器语句](assets/portfolio-name-filter.png)
      >
      >要排除处于“暂挂”状态的项目，必须应用以下过滤器：项目：状态不等于“暂挂”。 这会阻止保留项目中的工作项在工作负载均衡器中显示。
      >
      >![项目状态筛选器语句](assets/project-status-filter.png)
...
1. 单击&#x200B;**应用**&#x200B;以将过滤器的结果应用到所选的工作负载均衡器区域而不保存它。 左侧的工作项列表将更新。

   >[!IMPORTANT]
   >
   >当添加的所有过滤器语句同时为true时，结果将显示在工作负载均衡器中。

   该筛选器将保留到您刷新页面为止，并且&#x200B;**应用**&#x200B;按钮已替换为&#x200B;**另存为新按钮**。
...
1. 在&#x200B;**未分配的工作**&#x200B;或&#x200B;**已分配的工作**&#x200B;区域的右上角，单击&#x200B;**筛选器**&#x200B;图标![筛选器图标](assets/filter-icon.png)。  **筛选器**&#x200B;框显示在右侧，而要复制筛选器的区域名称显示在标题中。

1. 将鼠标悬停在现有筛选器上，然后单击&#x200B;**更多**&#x200B;菜单![更多](assets/more-menu-edit-filter.png)。
1. 单击&#x200B;**复制**。
...
1. 在&#x200B;**未分配**&#x200B;或&#x200B;**已分配的工作**&#x200B;区域的右上角，单击&#x200B;**筛选器**&#x200B;图标![筛选器图标](assets/filter-icon.png) 。 筛选器生成器将显示在右侧。

1. 将鼠标悬停在要编辑的筛选器上，然后单击&#x200B;**编辑**&#x200B;图标![编辑筛选器图标](assets/wb-edit-filter-icon.png)。
...
* 无法恢复已删除的筛选器。
* 您无法删除预定义过滤器。
* 您无法删除未保存的筛选器。 在注销并重新登录到Workfront后，它们会自动删除。
* 删除共享筛选器时，也将删除与其共享的所有用户的该筛选器。
...
1. 在&#x200B;**未分配的工作**&#x200B;或&#x200B;**已分配的工作**&#x200B;区域的右上角，单击&#x200B;**筛选器图标** ![筛选器图标](assets/filter-icon.png)。 **筛选器**&#x200B;框显示在右侧。

1. 将鼠标悬停在筛选器上，单击&#x200B;**更多**&#x200B;菜单![更多菜单](assets/more-menu-edit-filter.png)，然后单击&#x200B;**删除**。
...
1. 将鼠标悬停在筛选器上，然后单击&#x200B;**更多**&#x200B;菜单![更多](assets/more-menu-edit-filter.png)。

1. 单击 **Share**。显示&#x200B;**筛选器共享**&#x200B;框。
...
1. （可选）要编辑实体的筛选器权限，请单击其名称旁边的右箭头，然后选择&#x200B;**视图**&#x200B;或&#x200B;**管理**。

   ![筛选器权限](assets/access-level-additional-settings.png)

1. （可选）通过执行以下操作之一，启用或禁用实体的其他权限：

   1. 单击&#x200B;**查看**&#x200B;并禁用&#x200B;**共享**&#x200B;选项。 默认情况下启用此功能。

   1. 单击&#x200B;**管理**&#x200B;并启用&#x200B;**共享**&#x200B;或&#x200B;**删除**&#x200B;选项。

   >[!TIP]
   >
   >用户获得的权限不能高于其访问级别。 如果他们无权在其访问级别编辑“筛选器”，则他们将无权管理筛选器。 Workfront对这些用户禁用了“管理”选项。

1. 单击 **Share**。该筛选器与您指定的实体共享，并且共享筛选器显示在&#x200B;**筛选器**&#x200B;框的&#x200B;**与我共享**&#x200B;区域中。
