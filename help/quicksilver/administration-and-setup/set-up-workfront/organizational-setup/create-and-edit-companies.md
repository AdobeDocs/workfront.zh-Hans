...
通过访问[!UICONTROL 公司]页面，组管理员可以将组分配给公司，但该公司必须是组管理员创建的公司。 如果未将组管理员的访问级别配置为对公司的管理访问权限，则当组管理员创建公司时，需要[!UICONTROL 组]字段 — 其<span class="preview">星号</span>或粗体标题指示如下：

<span class="preview">预览环境中的示例图像：</span>
![需要与组的新公司对话框](assets/group-admin-add-company-group-required.png)

生产环境中的示例图像：
![组在公司](assets/group-admin-add-company.png)中是必需的
