...
title: 配置蓝图
description: 在安装Blueprint之前，可以配置项目模板或组织结构的详细信息。
author: Courtney
...
在安装蓝图之前，可以配置蓝图的详细信息。 项目模板和组织结构蓝图类型通常需要设置一些首选项和映射一些属性。 其他Blueprint类型可能不需要配置，您将以原样安装它们。 有关安装的更多信息，请参阅[安装Blueprint](/help/quicksilver/administration-and-setup/blueprints/blueprints-install.md)。
...
1. 指定以下内容：
   ...
   <table style="table-layout:auto">
    ...
    <tr>
        <td><strong>[!UICONTROL 项目群]</strong></td>
        <td>这是项目创建后所属的程序。</td>
    </tr>
   </table>

1. 选择模板是作为活动模板还是非活动模板安装。
1. 选择是否要使用定义的新问题首选项（如果首选项可用）。
   ...
   >[!TIP]
   >
   >* 使用“问题”首选项有助于为在项目中捕获新问题或请求的方式创建一致性。
...
某些模板包含规定的工作角色。 在将模板转换为项目时，职务角色可帮助您分配适当的人员。 在安装Blueprint之前，您可以自定义角色映射的方式。 单击&#x200B;**[!UICONTROL 查看角色描述]**&#x200B;以了解有关Blueprint中可用角色的更多信息。
...
1. 如果角色存在，您可以选择以下选项之一：
   ...
   1. 使用不同的名称创建新角色，然后在文本框中键入名称。
   1. 使用现有角色，然后在选择框中选择一个角色。
   1. 请勿使用映射的角色。 不建议使用此选项，因为某些任务将不会分配角色。
...
1. 如果公司存在，您可以选择以下选项之一：
   ...
   1. 使用现有公司，然后在选择框中选择公司。\
      ...
      蓝图中的主要公司映射到环境中的主要公司，即使它们没有相同的名称。
   1. 请勿使用映射的公司。 不建议使用此选项，因为其他对象中的公司引用将为空。
...
>[!NOTE]
>
>要在安装蓝图后配置公司，请参阅[安装蓝图后要采取的操作](../../administration-and-setup/blueprints/best-next-actions-after-install.md)。
...
一些Blueprint包含规定组。 组是一组用户，与您的部门结构一致。 组与Workfront中的团队和公司类似，但与之不同。 在安装Blueprint之前，您可以自定义组的映射方式。 单击&#x200B;**[!UICONTROL 查看群组描述]**&#x200B;以了解有关Blueprint中可用群组的更多信息。

Blueprint会按组名称搜索以查看是否存在任何匹配的现有组。 搜索区分大小写，因此名称必须完全匹配。 如果没有匹配的现有群组，您可以让Blueprint为您创建它们。

![[!UICONTROL 组映射]节](assets/Blueprints_GroupMapping.png)

1. 如果组存在，您可以选择&#x200B;**[!UICONTROL 重新映射组]**，然后选择以下选项之一：
