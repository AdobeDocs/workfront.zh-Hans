...
description: 您可以配置Adobe Workfront IP，将对Workfront的访问限制为您指定的75个IP地址或IP地址范围。 这为Workfront应用程序提供了额外的安全层。
author: Becky
...
您可以配置Adobe Workfront IP，将对Workfront的访问限制为您指定的75个IP地址或IP地址范围。 这为Workfront应用程序提供了额外的安全层。
...
将IP地址添加到Workfront后，只能使用这些IP地址来访问Workfront。 尝试从其他IP地址访问Workfront的用户会收到错误消息，指示其IP地址被阻止。
...
1. 指定当前用于访问Workfront系统的IP地址。
   ...
   在启用Workfront之前，必须将用于访问列入允许列表的IP地址添加到。
