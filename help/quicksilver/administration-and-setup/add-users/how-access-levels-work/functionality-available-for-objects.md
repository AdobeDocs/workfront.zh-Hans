...
只有具有Standard许可证的用户才能拥有项目组合的完全访问权限。
...
只有具有Standard许可证的用户才能拥有资源管理的完全访问权限。 其他许可证类型对Workfront中的资源管理的访问权限有限或没有访问权限。
