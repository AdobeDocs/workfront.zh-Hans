...
* [!UICONTROL AEM Assets]已安装并配置，版本6.5或更高版本。 有关安装[!UICONTROL AEM Assets]的信息，请参阅[[!DNL Adobe Experience Manager] 文档](https://experienceleague.adobe.com/docs/experience-manager.html)。
...
>[!IMPORTANT]
>
>以下说明适用于已由[[!DNL Workfront for Experience Manager] 增强型连接器](../../documents/workfront-and-experience-manager-integrations/workfront-for-experience-manager-enhanced-connector/workfront-for-aem-enhanced-connector.md)替换的[!DNL Workfront with AEM Assets]旧版连接器。 有关更多信息，请联系您的客户代表。

要安装[!DNL Workfront for AEM Assets]连接器，必须使用[!UICONTROL CRX包管理器]将该连接器作为包导入AEM。
...
1. 单击&#x200B;**[!UICONTROL 工具]** > **[!UICONTROL 部署]** > **[!UICONTROL 包]**。
   ...
   将打开[!UICONTROL CRX包管理器]。

1. 单击&#x200B;**[!UICONTROL 上传包].**

1. 在[!UICONTROL 上传包]对话框中，浏览并选择[!UICONTROL Workfront连接器]包，然后单击&#x200B;**[!UICONTROL 确定]**。\
   包显示在[!UICONTROL CRX包管理器]中。
...
1. 关闭[!UICONTROL CRX包管理器]。
   ...
   连接器已安装，您现在可以将[!DNL AEM Assets]配置为与[!DNL Workfront]集成。
...
1. 在左上角，从下拉菜单中选择&#x200B;**[!UICONTROL 用户]**&#x200B;，然后在&#x200B;**[!UICONTROL 搜索]**&#x200B;字段中输入&#x200B;*[!UICONTROL workfront-service]*。 选择[!UICONTROL workfront-service]用户。
1. 在屏幕右侧，选择&#x200B;**[!UICONTROL 添加ACE]**&#x200B;以创建新条目。
1. 在{&#x200B;0}添加新条目&#x200B;]**窗口&#x200B;中，选中**[!UICONTROL &#x200B;路径&#x200B;]**字段中的复选框&#x200B;图标，然后选择文件夹： */conf***[!UICONTROL 
1. 在“权限”字段中输入：*jcr：read*
...
1. 单击&#x200B;**[!UICONTROL 工具]** >**[!UICONTROL Cloud Service]**>**[!UICONTROL Workfront集成配置]** >**[!UICONTROL Global-Workfront].{8&#x200B;}**&#x200B;****

1. （视情况而定）如果您尚未这样做，请创建[!DNL Workfront]云配置文件。

   1. 单击[!DNL Global-Workfront]页面右上角的&#x200B;**[!UICONTROL 创建]**。
   1. 在&#x200B;**[!UICONTROL Workfront URL]**&#x200B;框中，指定[!DNL Workfront]实例的URL。

      例如，[!DNL https]：//`<account>`.my.workfront.com，其中`<account>`是您用于与AEM集成的帐户。

   1. 在{&#x200B;0}基本文件夹&#x200B;]**字段中，选中复选框图标，然后在下拉菜单中，选择链接到[!DNL Workfront]对象的文档的存储路径。**[!UICONTROL 
   1. 在显示的AEM模式窗口中，将包含连接到[!DNL Workfront]对象的文档的文件夹路径跟在后面。 选择文件夹，然后按右上角的&#x200B;**[!UICONTROL 选择]**&#x200B;。
   ...
   1. 在&#x200B;**[!UICONTROL Workfront API密钥]**&#x200B;框中，指定您的[!UICONTROL Workfront] API密钥。
      ...
      1. 单击&#x200B;**[!UICONTROL 系统]** >**[!UICONTROL 客户信息]**。
         ...
         如果您已生成API密钥，则[!DNL Workfront] API密钥将显示在用户的API密钥标签下。
      ...
      1. （视情况而定）如果您尚未生成API密钥，则需要生成一个：
         ...
         1. 在&#x200B;**[!UICONTROL 您用户的API密钥]**&#x200B;标签下，单击&#x200B;**[!UICONTROL 生成API密钥]**。

            [!DNL Workfront]的API密钥生成并显示。
      1. 将API密钥复制到剪贴板。
   ...
   1. 单击“Workfront集成配置”页面左上角的&#x200B;**[!UICONTROL 基本]**&#x200B;选项卡，然后单击&#x200B;**[!UICONTROL 连接]**。
...
1. 单击右上角的&#x200B;**[!UICONTROL 保存]**。

   将显示[!UICONTROL Global-[!DNL Workfront]]窗口。
...
1. （可选）同步AEM和[!DNL Workfront]之间的双向通信。
   ...
   1. 在窗口的左上角，单击&#x200B;**[!UICONTROL 属性]**。
      ...
      此时将显示[!UICONTROL [!DNL Workfront]集成配置]页。
...
[!UICONTROL AEM外部化器]允许AEM以可在[!DNL Workfront]中使用的格式传递URL。 如果未正确配置，[!DNL Workfront]将无法调用AEM API，并且在Workfront中关联AEM文档的URL将无法工作。

1. 在AEM中，单击&#x200B;**[!UICONTROL 工具]** > **[!UICONTROL 操作]** >**[!UICONTROL Web控制台]**。

1. 单击&#x200B;**[!UICONTROL OSGI]**，然后在下拉菜单中单击&#x200B;**[!UICONTROL 配置]**。

1. 在配置列表中，选择&#x200B;**[!UICONTROL Day CQ Link Externalizer].**

   显示[!UICONTROL 外部化器]页。

1. 在&#x200B;**[!UICONTROL 域]**&#x200B;部分中，确保[!UICONTROL 作者]字段中列出的域是AEM用户可从外部访问的域名。

   [!UICONTROL 作者]字段中的域名应与AEM实例的URL行中列出的域匹配。
...
1. 单击&#x200B;**[!UICONTROL 保存]**。
   ...
   [!UICONTROL AEM Assets]现已配置为将文档与[!DNL Workfront]链接
...
1. 以[!UICONTROL Workfront]管理员身份登录[!DNL Workfront]。
...
1. 在&#x200B;**[!UICONTROL 名称]**&#x200B;框中，指定自定义集成的名称。
   ...
   这是用户在[!UICONTROL Workfront]中使用集成时看到的名称；例如，您可以为名称输入&#x200B;*“[!DNL AEM Assets]”*。
...
1. （可选）确保集成标记为[!UICONTROL 活动]。\
   ...
   要在AEM中访问资源，必须将需要使用连接器的每个[!DNL Workfront]用户设置为AEM中的用户。 有关创建用户的信息，请参阅[设置用户以使用连接器](#set-up-users-to-use-the-connector)。
...
1. 以[!DNL Workfront]管理员身份登录[!DNL AEM Assets]。
1. 单击&#x200B;**[!UICONTROL 工具]** >{2&#x200B;}**{&#x200B;4}安全性]** >**[!UICONTROL 用户]**。****[!UICONTROL 
...
1. （视情况而定）如果用户在AEM中没有用户配置文件，请创建AEM用户配置文件。
   ...
   1. 输入用户的个人信息。
      ...
      唯一的必填字段是ID字段。 用户的AEM ID必须与其[!DNL Workfront] ID（用户的[!DNL Workfront]电子邮件地址）匹配。
