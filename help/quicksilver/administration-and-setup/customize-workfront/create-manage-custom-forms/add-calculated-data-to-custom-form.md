...
1. 在 **添加字段** 选项卡，单击 **已计算**.
   ...
   在右侧的显示区域中，将显示字段 *12345*. 这只是一个指示器，提醒您在创建或编辑自定义表单时该字段是一个计算自定义字段。 当表单附加到对象并且用户正在填写它时，他们会在字段中看到计算结果，而不是 *12345* 指示器。
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      ...
      <td> 默认情况下，您为该字段创建的公式存储在此处。 您可以添加文本以提供有关字段及其公式的附加信息。 这可以通过两种方式起作用： 
       <ul> 
        ...
        <li> <p>作为工具提示，用户将鼠标悬停在该字段上时即可看到。 您可以在此处添加任何您希望他们在工具提示中看到的文本。</p> <p>如果您不希望他们在工具提示中查看公式（这可能会让他们感到困惑），您可以隐藏该公式。 有关说明，请参阅一节中的表行“在说明中显示公式” <a href="#build-the-calculation-for-your-calculated-custom-field" class="MCXref xref">为计算的自定义字段构建计算</a> 本文章中。</p> </li> 
     ...
     <tr> 
      ...
      <td> <p>您希望存储并显示字段结果的格式。</p> <p>如果字段将用于数学计算，请始终使用 <strong>数字</strong> 或 <strong>货币</strong> 格式。 选择数字或货币时，系统会自动截断以0开头的数字。</p> 
...
1. 单击 **最大化** 以打开 **计算编辑器** 并构建您的计算。
   ...
   每个字段都必须用大括号括起来，如一节中所述 [计算自定义字段中必需的语法](#syntax-required-in-calculated-custom-fields) 本文章中。 开始键入字段名称时，系统会提供建议，您可以选择一项来将其插入到计算中。

   可在计算中引用任何类型的自定义字段，但以下两种类型除外：具有格式类型的文本字段和描述性文本。 有关自定义字段类型的信息，请参阅 [将自定义字段添加到自定义表单](../../../administration-and-setup/customize-workfront/create-manage-custom-forms/add-a-custom-field-to-a-custom-form.md)
...
1. 单击大文本框中的，然后单击 **表达式** 和 **字段** 以将其添加到您的计算中。
   ...
   > 
   >* 将鼠标悬停在计算中的表达式上可查看说明、显示如何使用表达式的示例以及指向文章中更多信息的“了解更多”链接 [计算的数据表达式](../../../reports-and-dashboards/reports/calc-cstm-data-reports/calculated-data-expressions.md).
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     <tr> 
      ...
      <td>您可以添加显示逻辑以确定是否根据用户在填写表单时在前面的多选字段（下拉列表、复选框或单选按钮）中所做的选择显示计算字段。 有关更多信息，请参阅 <a href="../../../administration-and-setup/customize-workfront/create-manage-custom-forms/display-or-skip-logic-custom-form.md" class="MCXref xref">将显示逻辑和跳过逻辑添加到自定义表单</a>. <p>仅当表单上计算的自定义字段前面至少有一个复选框、单选按钮或下拉字段时，此选项才可用。 </p> <p>跳过逻辑不可用于已计算的自定义字段。</p> </td> 
     </tr> 
     <tr> 
...
1. 要验证计算自定义字段是否正常工作，请将自定义表单附加到对象，然后查看计算自定义字段中的结果。
...
>[!NOTE]
>
>目前，系统仅在您开始键入要在自定义表单将附加到的对象上引用的字段名称时提出建议，而不是在对象的父级上提出建议。
...
* 如果您希望计算引用一个字段，该字段将从 *父级* 对象将自定义表单附加到对象时，您必须在字段名称前面加上父对象的对象类型，并且必须用大括号括起来。
   ...
   如果由于针对多个对象类型配置了的自定义，不确定父对象的对象类型是什么，则可以使用通配符筛选器变量 `$$OBJCODE` 允许计算适用于每种可能的类型。 有关更多信息，请参阅 [多对象自定义表单中的计算自定义字段](#calculated-custom-fields-in-multi-object-custom-forms) 本文章中。
...
这将确定以下内容：从自定义表单（任务）的对象中，您可以访问与任务（项目）相关的下一个对象。 从那里，您可以访问与项目（项目组合）相关的下一个对象，然后访问与项目组合（所有者）相关的下一个对象。

### 用于引用自定义字段的名称语法

在计算的自定义字段中引用另一个自定义字段时，您需要输入字段的名称，该字段会显示在Workfront用户界面中。
...
在多对象自定义表单中，选定的对象类型必须与表单的计算自定义字段中引用的所有字段兼容。 如果存在不兼容，则会显示一条消息，提醒您进行调整。

>[!INFO]
>
>**示例:**
>
>在配置为与Task对象类型一起使用的自定义表单中，您会创建一个名为“负责”的计算自定义字段。 您可以将其配置为引用内置字段，以便在将表单附加到任务时显示负责的主要被分配人的姓名：
>
>`{assignedTo}.{name}`
>
>之后，将Project对象类型添加到自定义表单。 出现一条警告消息，告知您Project对象类型与计算的自定义字段不兼容。
...
有关变量的更多信息，例如 `$$OBJCODE,` 参见 [通配符筛选器变量](../../../reports-and-dashboards/reports/reporting-elements/understand-wildcard-filter-variables.md).
