...
* **用户将使用Adobe标识登录。** Adobe将现有用户迁移到Adobe Admin Console后，用户将使用其新的Adobe身份(Adobe ID或Adobe Federated ID (SSO))登录到其Workfront实例。
...
* **Adobe将首先自动迁移Workfront管理员。** 在将Workfront管理员迁移到Adobe身份后，将为他们分配Adobe产品配置文件管理员（Workfront系统管理员）角色。 用户在迁移之前可能拥有的现有角色将不受影响。
...
* **在迁移用户时，这些用户将收到Adobe的电子邮件，通知他们登录Workfront的方式发生了更改。** 将邀请用户接受首次使用Adobe身份登录的邀请，方法是使用现有Adobe ID登录，或使用同一电子邮件地址设置新的Adobe ID。
