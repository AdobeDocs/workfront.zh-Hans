   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr>
      ...
      <td><p>单击<strong>[！UICONTROL新建评论]</strong>字段并键入您的评论。 使用格式设置工具设置文本的格式。 （图像在临时信息卡上的评论中不可用。） 要标记人员或团队，请使用评论区域底部的搜索框。 用户不必是讨论区成员。</p><p><strong>注意：</strong>此时，在临时卡上的注释中标记的用户不会收到电子邮件通知。
