...
title: 创建或编辑展示板
description: 从 [!UICONTROL 讨论区] 图标板中，您可以创建新展示板或编辑现有展示板。
author: Lisa
feature: Agile
...
要将展示板添加到工作流，请参阅 [管理工作流](/help/quicksilver/agile/use-boards-agile-planning-tools/manage-collections.md).
...
1. 要创建独立展示板，请单击 **[!UICONTROL 添加展示板]** 在 [!UICONTROL 讨论区] 区域。 要将展示板添加到工作流，请参阅 [管理工作流](/help/quicksilver/agile/use-boards-agile-planning-tools/manage-collections.md).

1. 为展示板选择模板。

   | 模板 | 描述 |
   |---------|----------|
   | 基本展示板 | 展示板上提供了三个默认列。 可以添加新列，也可以重命名或删除缺省列。 <p>展示板上提供了三个默认列。 可以添加新列，也可以重命名或删除缺省列。 |
   ...
   | 动态展示板 | 展示板上提供了以下列：“未选择”、“新建”、“进行中”、“已保留”和“完成”。 可以添加新列，也可以重命名或删除缺省列。 (可以重命名“未选择”列，但不能将其删除。 此列包含状态与任何其他列状态均不匹配的所有信息卡。) <p>默认列策略根据列的状态为列分配信息卡。 有关信息，请参阅 [管理展示板列](/help/quicksilver/agile/get-started-with-boards/manage-board-columns.md). |

1. 仅对于动态展示板，请执行设置向导步骤：
   ...
   1. 搜索并选择 [!UICONTROL **指定任务**] 将任务和问题提交到讨论区。
      ...
      此 [!UICONTROL **正在添加的卡片**] counter显示展示板上将有多少张卡。 例如，如果您选择具有100个任务和问题的项目，则计数器显示100。 如果您添加了一个用户分配，并且该人员被分配到项目中的5个任务，则计数器显示5。
   ...
   1. （可选）单击 [!UICONTROL **使用高级过滤器**] 以显示其他筛选器选项。
      ...
      此过程与在引入列上创建过滤器的过程相同。 有关更多信息，请参阅 [向展示板添加引入列](/help/quicksilver/agile/use-boards-agile-planning-tools/add-intake-column-to-board.md).
