...
1. 单击列表底部的&#x200B;**新行**。 此选项向表中添加一个新行。 在单元格中双击以输入信息。 每个单元格表示列表项的字段。 字段必须先存在，您才能在列表中看到它们。
   ...
   * 单行文本：您可以输入有限的字母数字信息。
   * 数字：必须以数字格式输入信息。
   * 货币：必须以数字格式输入信息，该格式前面带有货币符号。
   * 日期：使用日历选择日期。
   * 日期和时间：使用日历选择日期和时间。
   * 单选或多选下拉菜单：从下拉菜单中选择一个或多个选项。 <span class="preview">如果列表支持，则在找不到结果时，可以直接从下拉列表中创建新选项。</span>
   * 预输入：键入现有对象的名称（例如，组或项目的名称），并从建议列表中选择名称。
   * 段落：您可以输入多行字母数字信息。
   * 被分派人（一个或多个用户）：选择一个或多个用户作为列表项的被分派人。
   * 拾色器：从选取器中选择一种颜色，或键入代码，如十六进制或RGB。

   <div class="preview">

   Assignee、Single-select和Multi-select下拉字段支持以下键盘命令：

   * 使用键盘上的向上和向下箭头在项目列表中移动。
   * 按空格键选择项目。
   * 按Delete键删除选定项。
