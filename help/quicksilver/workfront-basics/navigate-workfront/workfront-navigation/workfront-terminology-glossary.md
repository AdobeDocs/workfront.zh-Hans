...
<p>[!UICONTROL FTE]必须是最多1的小数，并且不能是0。 </p>
   ...
   <li> 使用用户的计划时，Workfront会使用用户的休息时间、[!UICONTROL工作时间]值和[!UICONTROL默认计划]的小时数来计算用户的[!UICONTROL FTE]。 </li> </ul>
...
<p>表示用户可用于实际工作（不包括开销）的相当于全职([!UICONTROL FTE])的时间的百分比。 [!UICONTROL工作时间]必须是最多1的小数，并且不能是0。 例如，实际工作的20%可用性为0.2。</p>
   </p>字段的默认值为1，表示用户将其整个[!UICONTROL FTE]用于实际的项目相关工作。  </p>
   <p>系统使用此编号来计算用户在实际项目相关工作中的可用性。 </p>
   <p> 计划例外和休息时间也可能影响用户容量。 </p>
   ...
   <p>您可以在编辑或创建用户时更新用户的[!UICONTROL工作时间]。 有关信息，请参阅 <a href="../../../administration-and-setup/add-users/create-and-manage-users/edit-a-users-profile.md">编辑用户的配置文件</a></p> 
   <b>笔尖</b> 
   <p>将[!UICONTROL工作时间]值设置为1，表示用户可用于与项目相关的工作，且其整个全时对等内容。</p> 
   </td> 
  </tr> 
  <tr> 
   <td>[!UICONTROL工作时间]</td> 
