...
在將圖表新增至報表之前，您應該先為報表建立檢視和群組。 除非將資訊分組在報表中，否則無法新增大多數圖表。 唯一可以新增而不使用分組的圖表是量規圖表。\
...
如果您的報告顯示太多專案，則不會建立圖表。 在此情況下，您也必須新增篩選器至報表，以減少報表中的結果數量。\
...
1. 確保 **欄（檢視）** 索引標籤已更新，以符合報告的需求。\
   如需如何建立或修改報表檢視的詳細資訊，請參閱 [Adobe Workfront中的檢視概觀](../../../reports-and-dashboards/reports/reporting-elements/views-overview.md).
...
1. 按一下 **群組** 標籤並新增群組。
   ...
   >
   >* 您只能在報告結果分組時向報告新增圖表。
   >* 圖表不支援文字模式分組。 如需文字模式分組的詳細資訊，請參閱 [編輯群組中的文字模式](../../../reports-and-dashboards/reports/text-mode/edit-text-mode-in-grouping.md).
   >* 如果您新增代表一個量度的單一群組，則除了圓形圖之外，其他所有圖表都會將群組中的每個結果顯示為相同的顏色。
...
1. 開始將圖表新增至報表，如所述 [新增圖表至報表](#add-a-chart-to-a-report).
1. 在 **值** 欄位中，選取您要在報表上顯示的值，以及您想要如何摘要這些值。\
   在 **楔形** 欄位中，選取您要包含在圖表中的群組。 「群組」以圖表的楔形表示。

1. （選用）選取 **自訂顏色** 以自訂圖表上楔形的顏色。\
   如需自訂圖表顏色的詳細資訊，請參閱 [自訂圖表顏色](#customize-chart-colors).

1. （選用）選取 **以3D顯示** 以在3維檢視中顯示圖表。
1. 在 **將結果顯示為** 欄位中，選取結果在圖表中的顯示方式。 請考量下列選項：
...
1. 在 **底部(X)軸** 欄位中，選取您要包含在圖表中的群組。
1. （可選）選取顏色以自訂線條的顏色。
1. （選用）選取 **群組行**，為圖表選取其他群組。\
   （選用）選取 **自訂顏色** 以自訂新群組的顏色。\
   如需自訂圖表顏色的詳細資訊，請參閱 [自訂圖表顏色](#customize-chart-colors).

1. （選用）選取 **組合圖表** 以使用附加值來組合您的行。\
   請考量下列選項：
...
1. 開始將圖表新增至報表，如所述 [新增圖表至報表](#add-a-chart-to-a-report).
1. 在 **值** 欄位中，選取您要在報表上顯示的值，以及您想要如何摘要這些值。 如果您選取 **記錄計數**，顯示的值為報表的物件。

1. 在 **指標** 欄位中，選取您要包含在圖表中的群組。 「群組」由圖表上的指標線表示。\
   如果您有包含兩個專案的「群組」，圖表上會顯示兩個指標。\
   例如，如果您有專案狀態群組，並且有兩個專案狀態（「目前」和「暫停」），您的量測軌圖表包含兩個量測軌指標。 他們會指向處於該狀態的專案數量。\
   （選用）選取 **總計** 在 **指標** 欄位，顯示所選物件的總數。 **值** 欄位。

1. 在 **值範圍** 欄位中，指定值的範圍和顏色，以代表要在量測軌圖表中顯示的值。
1. （可選）按一下 **新增另一個值範圍** 以新增其他值範圍至圖表。\
...
您最多可以在中顯示一個物件的三個欄位 **泡泡** 圖表。 這表示您最多可以在泡泡圖中顯示四個資料點。 具有三個關聯欄位的每個圖元都會顯示為圓，表示其在X和Y軸內的位置內的兩個欄位。 第三個欄位由圓形大小表示。
...
1. 在 **底部(X)軸欄位**&#x200B;中，選取要包含在圖表X軸上的值。 值來自報表的檢視。 指定您要如何摘要資訊。
   ...
   >[!NOTE]
   >
   >請確定您至少有一個針對此欄位進行摘要的欄位處於作用中狀態。\
   >如需彙總報告欄中資訊的詳細資訊，請參閱 [建立自訂報表](../../../reports-and-dashboards/reports/creating-and-managing-reports/create-custom-report.md).

1. 在 **泡泡大小** 欄位中，選取您想要以圖表中的泡泡大小表示的值。 值來自報表的檢視。 指定您要如何摘要資訊。
...
您可以讓Workfront選取圖表中元素的顏色，也可以在將圖表新增至報表時加以自訂。 如果您的圖表包含代表一個量度的單一「群組」（例如顯示按實際完成日期分組的任務數量的任務報告），則「群組」中的每個結果都會以相同的顏色顯示。

您只能為報表檢視中顯示的欄位選擇一個顏色。 您可以為顯示在報表「分組」中的欄位選擇幾種顏色（每個選項一種）。
...
1. 按一下 **自訂顏色** 此欄位可用時。\
   ...
   >[!NOTE]
   >
   >您可以將自訂顏色與任何可依據的欄位建立關聯，並將自訂顏色與某些可顯示在檢視中的欄位建立關聯，包括自訂欄位。 您在「自訂顏色」對話方塊中選擇的自訂欄位或自訂欄位選項會區分大小寫。

1. 請考慮選取下列任一選項：

   * **使用一種顏色**：圖表的所有元素都會以選取的顏色顯示。

      1. 開始輸入所選欄位的選項名稱，然後選取顏色。 此選項會以圖表上選取的顏色顯示。
      1. （可選）為您的顏色指定十六進位顏色值，而不是從可用的顏色範例中選取一個值\
         或\
         按一下按一下十六進位程式碼後顯示的檢色器，然後選取其他顏色。
   * **新增色彩**：繼續為所選欄位的所有其他可能選項新增自訂顏色。
   * **全部移除**：選取此選項可移除上方選取欄位的所有顏色和選項。
   * **進階選項**：從下列選項中選取：

      * **無值**：選取此欄位和自訂顏色，以顯示將「無值」專案分組的圖表欄。 這些專案無法依您在分組中所選欄位的任何選項分組。
