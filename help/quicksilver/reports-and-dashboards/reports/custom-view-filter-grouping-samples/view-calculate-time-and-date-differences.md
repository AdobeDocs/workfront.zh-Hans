...
title: 「檢視：計算時間和日期差異」
description: 您可以計算下列專案之間的差異 — 編輯我。
...
>[!IMPORTANT]
>
>您無法在Adobe Workfront中計算相同型別兩個不同物件之間的時間和日期差異。 例如，您無法計算兩個不同專案、任務或問題中兩個日期之間的時間和日期差異。
...
1. 按一下 **新增欄** 並開始輸入「實際完成日期」 **顯示在此欄中** 欄位，然後在清單中顯示時選取它。

1. 按一下 **新增欄**，然後按一下 **切換至文字模式**.
...
## 計算物件上的欄位與父物件上另一個欄位之間的時間和日期差異

如需物件及其父項的清單，請參閱以下主題中的「瞭解物件的相依性和階層」一節： [瞭解Adobe Workfront中的物件](../../../workfront-basics/navigate-workfront/workfront-navigation/understand-objects.md).\
例如，您可以計算任務的計畫完成日期與其父任務或任務所在專案的計畫完成日期之間的差異。
