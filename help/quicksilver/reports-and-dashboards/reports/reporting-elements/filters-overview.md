...
>[!NOTE]
>
>本文介绍了可在Workfront中应用过滤器的所有区域，以及有关列表和报告中的过滤器的一般信息。 请参阅下面链接的文章，以了解有关列表和报告以外的特定区域中过滤器的信息。

## Workfront过滤器的类型

Workfront中有多种类型的过滤器。 所有筛选器都可达到限制屏幕上显示的信息量的相同目的。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td>由Workfront构建或由您或其他用户自定义。 您可以多次使用它们。 如果您拥有正确的访问权限，则可以在多个区域命名、保存和重用这些权限。 您还可以与其他用户共享这些过滤器。 它们可在对象列表或报告中使用。 </td> 
  </tr> 
  <tr> 
...
* 您可以自定义列表和报表中的现有筛选器。 所有有权访问筛选器的用户也可以看到您的更改。
...
构建过滤器时，将构成过滤器语句的多个元素连接在一起。 您可以使用多个过滤器语句来定义过滤器的条件。
...
例如，在名为“Color”的项目中有一个单选按钮字段，其选项为：
...
例如，如果您有一个包含3个选项（A、B和C）的字段，并在对象上3个选项（A和B，但不是C）中选择了2个，则您可以创建一个带有筛选器的报告，其中选项A和B的限定符为“不等于”，并且筛选掉同时选择了A和B，但不限于A和B的项目。 如果为选项A创建具有“不等于”限定符的过滤器，则它只会过滤掉仅选定了A的对象，但如果选定了A和B，这些对象仍会显示在报表中。
