...
您可以使用以下界面在选定列表中共享过滤器：

* 标准界面
* 测试版生成器界面

根据您用于从哪个界面共享过滤器，在选定列表中共享过滤器会有所不同。 有关过滤器构建界面类型的信息，请参阅 [在Adobe Workfront中创建或编辑过滤器](/help/quicksilver/reports-and-dashboards/reports/reporting-elements/create-filters.md).

您只能在标准界面中共享视图和分组。

* [使用标准界面共享过滤器、视图和分组](#share-with-standard-interface)
* [使用测试版生成器界面共享过滤器](#share-with-beta-builder-interface)

### 使用标准界面共享过滤器、视图和分组 {#share-with-standard-interface}

在标准界面中共享过滤器、视图和分组是相同的。
...
在共享项目、任务或问题列表中的过滤器时，您可以使用测试版生成器界面而不是标准界面来共享它们。

测试版生成器界面不适用于Workfront中的任何其他对象。

生成报表时，无法在测试版生成器界面中生成过滤器。
...
1. 单击 **过滤器** 图标 ![“过滤器”图标](assets/filter-nwepng.png)，然后启用 **测试版设置** ![测试版设置](assets/beta-toggle-white-on-existing-filters.png) 以访问测试版生成器。 默认情况下，该复选框处于禁用状态。
   ...
   >[!TIP]
   >
   >启用测试版生成器后，生成器界面的标题将变为蓝色。 启用测试版生成器界面后，Workfront会保留所有可用区域的该界面处于启用状态。
...
1. 启用查看系统宽设置。 这会为Workfront中的任何人授予查看过滤器的权限。
