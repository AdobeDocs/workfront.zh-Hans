...
通过有条件地设置报告视图中的列的格式，您可以设置影响报告显示方式的规则。 当满足这些条件或规则时，将应用特殊格式设置。

例如，如果任务的完成百分比低于20%，则可以通过以粗体、红色文本和黄色背景颜色显示百分比数来突出显示字段。
...
在报告视图中所做的更改仅在报告的“详细信息”选项卡中生效。 这些更改不影响报告的“摘要”、“矩阵”或“图表”选项卡。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>对筛选器、视图、组的编辑访问权限</p> <p>编辑对报告、功能板、日历的访问权限以编辑报告中的视图</p>
   </td> 
  </tr> 
  <tr> 
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     <tr> 
      ...
      <td> <p>指定列的名称。</p> <p>如果正在编辑现有列，则在此处指定名称将更改现有列名称。</p> </td> 
     </tr> 
     <tr> 
...
1. 在“**显示如下字段：**”部分中，指定当满足上面定义的条件时，此字段的外观。
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     <tr> 
      ...
      <td> <p>使用颜色选择器选择文本显示的颜色。</p> <p><b>注释</b></p> <p> 如果该字段包含超链接，则文本颜色选择项不会应用于该字段。</p> </td> 
     </tr> 
     <tr> 
     ...
     <tr> 
      ...
      <td> <p>选择此选项可显示此列的自定义标签，而不是其实际值。 指定要显示的文本，而不是提供的字段中的值。</p> <p><b>重要</b></p> <p>选择“<strong>显示文本</strong>”将禁用内联编辑此列中的文本的功能。<br>此外，无法更改“前置任务”列的值，因为它包含内置逻辑。</p> </td> 
     </tr> 
     <tr> 
      <td role="rowheader"><strong>应用于整行</strong></td> 
      <td>选择此选项可将设置应用于整行，而不只是将设置应用于选定列。</td> 
     </tr> 
    </tbody> 
   </table>
...
1. 单击&#x200B;**添加规则**。\
   ...
   **示例1**
   ...
   您可以首先创建一个规则，说明项目处于“构建状态”时，文本颜色为紫色和粗体。 然后，创建第二个规则，规定当任务的“名称”不为空时，文本颜色为红色和斜体，背景颜色为绿色。 在此示例中，将发生以下情况：

   * 项目状态为“正在生成”的任务以紫色和粗体文本显示。 如果任务名称不为空，则任务的背景也会呈绿色。
   * 项目状态为生成以外的任何任务（任务名称不为空）将以红色和斜体文本显示，背景为绿色。

   **示例2**

   在项目计划完成日期列中创建一个影响整行的规则，如果项目已取消（例如，项目状态为“废弃”），则变为背景灰色。 然后，创建一个列规则，当项目计划完成日期早于今天（即项目延迟）时，该列规则将变为背景红色。 在此示例中，如果取消的项目具有延迟完成日期，则该单元格将显示为红色，即使行中的其他单元格为灰色。 要更正此格式，请执行以下操作：

   * 编辑计划完成日期的格式，并删除延迟项目中红色背景的列规则。
   * 添加与行规则格式相同的列规则（项目状态=无效时为灰色背景）。
   * 为后期项目的红色背景再次添加列线。
   * 保存规则和视图时，红色背景不适用于已取消的项目。

1. 单击&#x200B;**保存**。
1. 单击&#x200B;**保存+关闭**。\
   在报告中，如果满足指定的条件，用户会看到对格式所做的更改。
