...
* 在文本模式下开始构建报表元素之前，您必须先了解Adobe Workfront语法。 文本模式的Workfront语法是此应用程序所特有的，并具有您必须熟悉的独特特征。
* 在报表中开始使用文本模式之前，我们强烈建议您参加我们的高级报表课程，以更深入地了解我们的文本模式语言。 <!--outdated link: For training materials on reporting see [Workfront Reports and Dashboards Learning Paths](https://one.workfront.com/s/learningpath2/workfront-reporting-20Y0z000000blhLEAQ).-->
...
* 请记住Workfront中的对象层次结构。 视图、筛选器和分组之间存在以下差异：
   ...
   * 您可以显示一个对象，该对象与视图中的报告或列表对象相距三个对象。
  ...
  **示例：** 您可以在任务视图中显示Portfolio所有者的名称或GUID：
  ...
  您不能在Portfolio视图中对“任务所有者”进行分组、筛选或提示：

  `project:portfolio:ownerID=5808f4bc00790b270a9629dd128e63fa`


  在这些示例中，Portfolio所有者ID是距离列表对象之外的三个对象。
...
* 尽可能使用通配符，以使报告和列表更加动态，并避免为不同用户和类似的时间线重复使用它们。

## 驼峰式大小写概述

在文本模式下引用Workfront字段或其属性时，Workfront要求您以驼峰式大小写键入其名称。 在这种情况下，单名字段的拼写会小写。 复合字段的拼写遵循以下模式：
...
**示例：** 要引用项目的实际完成日期，您在构建文本模式报告元素时可以使用的字段名称是
...
视图或分组最重要的代码行是标识视图列或分组中引用的对象的行。 根据此字段是直接引用Workfront数据库字段还是多个字段之间的计算，代码行可能以 `valuefield` 或 `valueexpression`
...
>  
>* 如果在同一列中编辑视图中的多个列（与共享列的情况相同），请记住，每个列的每行代码都以列号开头。
>
>  使用以下格式标识视图的第一列：
>
...
* 在引用自定义字段时，请完全按照界面中显示的字段名称来使用字段名称。
...
>[!TIP]
>
>尽管您可以构建可在报表中显示的计算字段，但计算视图和分组更加动态。 每次运行报表或显示列表时，计算视图和分组都会刷新并包含新信息。
>
>有关在视图中创建计算列的信息，请参见 [计算自定义字段与计算列](../../../reports-and-dashboards/reports/calc-cstm-data-reports/calculated-custom-fields-calculated-columns.md).
...
* 使用驼峰式大小写直接引用字段并用大括号括住每个字段。
  ...
  **示例：** 要使用，在任务列中显示任务名称字段 `valueexpression`，请使用以下行：
...
* 引用自定义字段时，使用以下规则：
  ...
  **示例：** 要在valueexpression行的任务视图中显示“其他详细信息”项目自定义字段，请使用以下行：
...
视图或分组中第二最重要的代码行是 `valueformat=` 行。 这会告知Workfront采用什么格式返回您在
`valuefield` 或valueexpression行。 尽管您可以对以下内容使用各种格式 `valueformat` 行，我们建议在使用时
`valueexpression`：
...
`width=` 是一行代码，您可以在其中指定每列的宽度（以像素为单位）。 Workfront为每个字段提供了建议的宽度，不过根据字段类型和格式，您可能需要进行调整。
...
当您使用附加代码行时，将强制执行列的延伸： `usewidths=true` 视图中至少有一列的属性。

**示例：** 要指示列可以在视图中使用70%的空格，请使用以下行：
...
创建过滤器的语法与创建自定义提示的语法相似。

>[!TIP]
>
>您可以首先为要包含在提示中的语句构建过滤器，从而创建自定义提示。 使用“&amp;”连接筛选器中的所有代码行，且各行之间没有任何空格，这会成为您的自定义提示。
...
* 通配符，使过滤器更加动态并针对当前时间或登录的用户自定义过滤器。 有关通配符的信息，请参见 [通配符筛选器变量](../../../reports-and-dashboards/reports/reporting-elements/understand-wildcard-filter-variables.md).
