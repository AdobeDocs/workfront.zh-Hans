---
product-area: Canvas Dashboards
navigation-topic: report-types
title: 在画布功能板中构建数据透视表
description: 您可以将数据透视表添加到画布仪表板，以表格式查看数据的聚合总计。
author: Courtney
feature: Reports and Dashboards
source-git-commit: 58c5f4a08b2083d7350c19b6c1d8383fa0dbb124
workflow-type: tm+mt
source-wordcount: '1556'
ht-degree: 5%

---

# 在画布功能板中构建数据透视表

>[!IMPORTANT]
>
>画布功能板目前仅适用于参与Beta测试阶段的用户。 在此阶段，部分功能可能无法完成或无法按预期工作。 请按照“画布功能板测试版”概述文章中[提供反馈](/help/quicksilver/product-announcements/betas/canvas-dashboards-beta/canvas-dashboards-beta-information.md#provide-feedback)部分的说明提交任何有关您体验的反馈。<br>
>如果您对可能的错误或技术问题有反馈，请向Workfront支持提交票证。 有关详细信息，请参阅[联系客户支持](/help/quicksilver/workfront-basics/tips-tricks-and-troubleshooting/contact-customer-support.md)。<br>
>请注意，以下云提供商未提供此测试版：
>
>* 自带Amazon Web Services密钥
>* Azure
>* Google Cloud Platform

可以将数据透视表添加到画布仪表板，以表格式查看数据的聚合总计，例如总和、计数和平均值。 当针对多个维度比较多个聚合值或计数时，数据透视表非常有用。

![数据透视表示例](assets/pivot-table-example.png)

## 访问要求

+++ 展开可查看本文所述功能的访问权限要求。

<table style="table-layout:auto"> 
<col> 
</col> 
<col> 
</col> 
<tbody> 
<tr> 
   <td role="rowheader"><p>Adobe Workfront 包</p></td> 
   <td> 
<p>“任一” </p> 
   </td> 
<tr> 
 <tr> 
   <td role="rowheader"><p>Adobe Workfront许可证</p></td> 
   <td> 
<p>标准</p> 
<p>规划</p> 
   </td> 
   </tr> 
  </tr> 
  <tr> 
   <td role="rowheader"><p>访问级别配置</p></td> 
   <td><p>编辑对报告、功能板和日历的访问权限</p>
  </td> 
  </tr>  
</tbody> 
</table>

有关此表中信息的更多详细信息，请参阅Workfront文档中的[访问要求](/help/quicksilver/administration-and-setup/add-users/access-levels-and-object-permissions/access-level-requirements-in-documentation.md)。
+++

## 先决条件

必须先创建仪表板，然后才能构建数据透视表。 有关详细信息，请参阅[创建画布功能板](/help/quicksilver/reports-and-dashboards/canvas-dashboards/create-dashboards/create-dashboards.md)。

## 在画布功能板中构建数据透视表

有许多配置选项可用于构建数据透视表。 在本节中，我们将引导您完成创建规则库的常规过程。

{{step1-to-dashboards}}

1. 在左侧面板中，单击&#x200B;**画布功能板**，然后单击要将报告添加到的功能板的名称。

1. 单击页面右上角的&#x200B;**添加报告**。

1. 在&#x200B;**添加报告**&#x200B;框中，选择&#x200B;**创建报告**。

1. 在左侧，选择&#x200B;**数据透视表**。

1. 单击右上角的&#x200B;**创建报告**。

1. （可选）请按照以下步骤配置&#x200B;**详细信息**&#x200B;部分：

   1. 为报表选择&#x200B;**根实体**。

      >[!NOTE]
      >
      > 根实体设置字段来自的对象。 选定后，此报表中稍后使用的每个字段选择器都将从该对象开始，因此您可以直接转到所需的字段。


   1. 输入报告&#x200B;**名称**。

   1. 输入报告&#x200B;**描述**。

   1. （可选）在&#x200B;**使用**&#x200B;的访问权限运行此报告字段中，开始键入您希望报告使用的权限的用户名称，然后在用户出现在列表中选择该用户。 当您将报表配置为以其他用户身份运行时，仪表板的所有查看者都会看到相同的数据，无论他们自己的访问级别如何。 如果不选择用户，则每个查看者将根据自己的权限查看数据。

      >[!IMPORTANT]
      >
      >如果所选用户被停用或无法访问相关工作区或记录类型，则报告可能会显示不完整的数据或无法呈现。

1. 请按照以下步骤配置&#x200B;**指标**&#x200B;部分：

   1. 在左侧面板中，单击&#x200B;**显示量度** ![生成KPI图标](assets/build-kpi-icon.png)图标。

   1. 单击&#x200B;**添加量度**，然后选择所需的字段。 该字段将作为列显示在右侧的预览部分中。

      >[!NOTE]
      >
      > 量度（也称为度量）是要相加或总计的数字字段。 例如，您可以合计所有成本，或计算任务数量。


   1. 输入&#x200B;**列标签**。

   1. 在&#x200B;**聚合类型**&#x200B;下拉列表中，选择该字段的数据汇总方式。 此字段中的选项因您选择的字段类型而异。

   1. 对要添加的每个量度重复以上两个步骤。

1. 按照以下步骤配置&#x200B;**区段**&#x200B;部分：

   1. 在左侧面板中，单击&#x200B;**区段** ![深入分析组图标](assets/drilldown-group-icon.png)图标。

   1. 单击&#x200B;**添加区段**，然后选择所需的区段。 该字段将作为列显示在右侧的预览部分中。

      >[!NOTE]
      >
      >区段是用于分组数据的类别，例如按状态或按所有者分组任务。 这是量度的排序和汇总方式。


   1. 重复上述两个步骤以添加最多2个区段。

1. 按照以下步骤配置&#x200B;**筛选器**&#x200B;部分：

   1. 在左侧面板中，单击&#x200B;**筛选器** ![筛选器图标](assets/filter-icon.png)图标。

   1. 选择&#x200B;**编辑筛选器**。

   1. 单击&#x200B;**添加条件**，然后指定要作为筛选依据的字段以及定义该字段必须符合何种条件的修饰符。

   1. （可选）单击&#x200B;**添加筛选器组**&#x200B;以添加另一组筛选条件。 集合之间的缺省运算符是AND。 单击运算符以将其更改为OR。

1. 按照以下步骤配置&#x200B;**明细列设置**&#x200B;部分：

   1. 在左侧面板中，单击&#x200B;**向下钻取列** ![向下钻取列图标](assets/drilldown-column.png)图标。

   1. 单击&#x200B;**添加列**，然后选择要显示为深入分析表中列的字段。 对要添加的每个列重复此过程。

1. 单击&#x200B;**保存**&#x200B;以创建报告并将其添加到仪表板。

## 构建数据透视表示例

在本节中，我们将介绍创建汇总任务完成数据的数据透视表的步骤。

{{step1-to-dashboards}}

1. 在左侧面板中，单击&#x200B;**画布功能板**，然后单击要将报告添加到的功能板的名称。

1. 单击页面右上角的&#x200B;**添加报告**。

1. 在&#x200B;**添加报告**&#x200B;框中，选择&#x200B;**创建报告**。

1. 在左侧，选择&#x200B;**数据透视表**。

1. 单击右上角的&#x200B;**创建报告**。

1. 按照以下步骤配置&#x200B;**详细信息**&#x200B;部分：

   1. 选择&#x200B;**任务**&#x200B;作为&#x200B;**根实体**。
   1. 在&#x200B;**名称**&#x200B;字段中键入&#x200B;*按项目组合和项目*&#x200B;计划的任务小时数和实际小时数。
   1. 在&#x200B;**描述**&#x200B;字段中键入描述。

1. 请按照以下步骤配置&#x200B;**指标**&#x200B;部分：

   1. 在左侧面板中，单击&#x200B;**显示量度** ![生成KPI图标](assets/build-kpi-icon.png)图标。
   1. 单击&#x200B;**添加指标**，然后选择&#x200B;**名称**。 在&#x200B;**列标签**&#x200B;字段中键入&#x200B;*任务计数*。 在&#x200B;**聚合类型**&#x200B;下拉列表中，选择&#x200B;**计数**。
   1. 单击&#x200B;**添加指标**，然后选择&#x200B;**实际小时数**。 在&#x200B;**列标签**&#x200B;字段中键入&#x200B;*实际小时数*。 在&#x200B;**聚合类型**&#x200B;下拉列表中，选择&#x200B;**总和**。
   1. 单击&#x200B;**添加指标**，然后选择&#x200B;**计划小时数**。 在&#x200B;**列标签**&#x200B;字段中键入&#x200B;*总计划小时数*。 在&#x200B;**聚合类型**&#x200B;下拉列表中，选择&#x200B;**总和**。

1. 按照以下步骤配置&#x200B;**区段**&#x200B;部分：

   1. 在左侧面板中，单击&#x200B;**区段** ![深入分析组图标](assets/drilldown-group-icon.png)图标。
   1. 单击&#x200B;**添加区段**，然后选择&#x200B;**项目** > **Portfolio** > **名称**。
   1. 单击&#x200B;**添加区段**，然后选择&#x200B;**项目** > **名称**。

1. 按照以下步骤配置&#x200B;**筛选器**&#x200B;部分：

   1. 在左侧面板中，单击&#x200B;**筛选器** ![筛选器图标](assets/filter-icon.png)图标。
   1. 选择&#x200B;**编辑筛选器**，然后选择&#x200B;**添加条件**。
   1. 单击空条件筛选器，然后单击&#x200B;**选取字段**。
   1. 选择&#x200B;**状态**。
   1. 将运算符更改为&#x200B;**等于**，然后选择&#x200B;*进行中*。

1. 按照以下步骤配置&#x200B;**明细列设置**&#x200B;部分：

   1. 在左侧面板中，单击&#x200B;**向下钻取列** ![向下钻取列图标](assets/drilldown-column.png)图标。
   1. 单击&#x200B;**添加列**，然后选择&#x200B;**名称**。
   1. 单击&#x200B;**添加列**，然后选择&#x200B;**分配给** > **名称**。
   1. 单击&#x200B;**添加列**，然后选择&#x200B;**计划完成日期**。

1. 单击屏幕右上角的&#x200B;**保存**。

## 构建数据透视表时的注意事项

### 带有财务数据的报表

在访问级别对财务数据具有“查看”或“编辑”权限的用户仍可在画布功能板可视化图表中查看财务数据，即使已在任务或项目级别删除了“查看财务”权限。

* 在访问级别中不具备财务数据权限的用户，将无法在报告中看到财务数据。
* 能够看到财务数据的用户，仅限于查看其本身已有权限访问的记录（如项目、任务、问题等）。 对于无权访问的记录，用户将无法看到对应的财务数值。
* 报告创建者在将财务数据包含到仪表板中时应谨慎，并注意仪表板的共享对象，以避免发生非预期的访问。

这是一个已知的限制，我们计划在将来解决它。

### 利用字段选择器

**生成透视表**&#x200B;节中的&#x200B;**节**&#x200B;下拉列表旨在缩小字段选择器中的选择范围，以便在生成透视表时更容易查找对象。 要开始，请选择一个基本实体对象。

* **所有节**： Workfront和Workfront Planning中的所有对象类型。
* **Workfront对象**：本机Workfront对象。
* **Planning记录类型**： Workfront Planning中定义的自定义记录类型。

![分区下拉列表](assets/sections-dropdown.png)

选择基本实体对象后，**节**&#x200B;下拉列表会更新为可用的字段类型选项以供选择。

* **所有节**：本机字段、自定义字段和相关对象。
* **所有字段**：本机字段和自定义字段（不包括关系）。
* **自定义字段**：自定义表单或Planning记录中的客户定义字段。
* **Workfront字段**：仅本机字段。
* **关系**：连接的记录。

![可报告对象选择](assets/reportable-objects-selection.png)

### 引用相关对象

我们限制了对选择子对象作为数据透视表区段的访问权限。 区段选项可以是记录本身或不表示1:many或多:many关系的其它相关记录的属性。

我们还限制将任何父属性或子属性引用为量度的访问权限，以减少重复计数或重复汇总值的可能性，从而导致实际数据的误报。

