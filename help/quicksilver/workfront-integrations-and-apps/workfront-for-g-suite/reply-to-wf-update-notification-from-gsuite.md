---
product-area: workfront-integrations
keywords: google，doc，document，sheet，幻灯片
navigation-topic: workfront-for-g-suite
title: 回复 [!DNL Adobe Workfront] 来自G Suite的更新通知
description: 当您在 [!DNL Adobe] [!DNL Workfront]，您可以快速发布Workfront for G Suite的回复。
author: Becky
feature: Workfront Integrations and Apps
exl-id: b39c53b5-02b6-4c13-9f25-83cb3e3096d8
source-git-commit: 3143e5a4988b7234d8225da442f5af1d756d461d
workflow-type: tm+mt
source-wordcount: '199'
ht-degree: 1%

---

# 回复 [!DNL Adobe Workfront] 更新通知 [!DNL G Suite]

当您在 [!DNL Adobe] [!DNL Workfront]，您可以快速发布来自 [!DNL Workfront for G Suite].

## 访问要求

您必须具有以下访问权限才能执行本文中的步骤：

<table style="table-layout:auto"> 
 <col> 
 <col> 
 <tbody> 
  <tr> 
   <td role="rowheader">[!DNL Adobe Workfront] 计划*</td> 
   <td> <p>任意</p> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">[!DNL Adobe Workfront] 许可证*</td> 
   <td> <p>[!UICONTROL Work]、[!UICONTROL Plan]</p> </td> 
  </tr> 
  </tbody> 
</table>

&#42;要了解您拥有的计划、许可类型或访问权限，请联系您的 [!DNL Workfront] 管理员。

## 先决条件

在创建 [!DNL Workfront] 任务 [!DNL G Suite]，您必须

* 安装 [!DNL Workfront for G suite]\
   有关说明，请参阅 [安装 [!DNL Adobe Workfront for G Suite]](../../workfront-integrations-and-apps/workfront-for-g-suite/install-workfront-for-gsuite.md).

## 回复来自 [!DNL G Suite]

1. 如果 [!UICONTROL Workfront for G Suite] 未显示面板，请单击 [!DNL Workfront] 图标 ![](assets/wf-lion-icon.png) 在 [!DNL G Suite] 页面最右侧的附加组件侧栏。
1. 打开 [!DNL Workfront] 包含更新的电子邮件通知。
1. 在 [!UICONTROL Workfront for G Suite] 面板，单击 **[!UICONTROL 发布评论]**，然后键入您的回复。
1. （可选）要通知特定用户您的回复，请单击 **[!DNL Notify people and teams]**，开始键入名称，然后在下拉列表中显示该名称时单击该名称。

   您可以对要添加的任何其他名称重复此步骤。

1. 单击 **[!DNL Post]**.
