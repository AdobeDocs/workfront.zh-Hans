...
有关对象、有效对象代码和对象字段的列表，请参阅[API资源管理器](../../wf-api/general/api-explorer.md)。
...
<pre>GET /attask/api/&lt;supported-version&gt;/project？id=4c78...54d0&amp;method=get<br>GET /attask/api/&lt;supported-version&gt;/project/4c78...54d0？method=get</pre>
...
已为PUT、POST和DELETE请求添加了特殊安全性。 只有在URI中包含&#x200B;**sessionID=abc123**&#x200B;时，才能执行导致写入数据库或从数据库中删除的任何请求。 以下示例显示了如何查找DELETE请求：
<pre>GET /attask/api/&lt;supported-version&gt;/project？id=4c78...54d0&amp;method=delete&amp;sessionID=abc123<br>GET /attask/api/&lt;supported-version&gt;/project/4c78...54d0？method=delete&amp;sessionID=abc123</pre>
...
>[!NOTE]
>
>本节中介绍的过程仅适用于尚未载入Adobe Business Platform的组织。 由于所有组织现已登记到Adobe Business Platform，因此&#x200B;**通过Workfront API登录Workfront不再可用** 。
>
...
1. 导航到登录屏幕，但不登录。
1. 将URL更改为/attask/api/&lt;supported-version>/project/search。\
   请注意，找不到该页面。
1. 将&#x200B;*搜索*&#x200B;一词替换为login？username=admin&amp;password=user，将您的用户名和密码替换为&#x200B;*admin*&#x200B;和*user\
   *此会话将作为Cookie存储在浏览器中，无需在后续每个GET请求中重述。

1. 将URL更改回&#x200B;**/attask/api/&lt;supported-version>/project/search**。
1. 请注意提供的响应。

在执行PUT、POST和DELETE请求时，必须始终包含登录后提供的sessionID。
...
请注意/attask/api/&lt;supported-version>/project？id=...请求与`/attask/api/&lt;supported-version&gt;/project/...`请求相同。
...
<pre>GET /attask/api/&lt;supported-version&gt;/task/search？name=Planning<br>&amp;name_Mod=contains<br>&amp;OR:1:portfolio：name=FixedAssets<br>&amp;OR:1:portfolio：name_Mod=eq<br>&amp;OR:1:assignedTo：name=Steve<br>&amp;OR:1:assignedTo：name_Mod=cicontains<br>&amp;OR:2:parent：name=Final Task<br>&amp;OR:2:父项：name_Name_Name mod=eq
</pre>
...
<pre>/attask/api/&lt;supported-version&gt;/task/search？map=true</pre>返回按ID索引的响应，如下所示：
<pre>{<br> “数据”： {<br> “4c9a97db0000000f13ee4446b9aead9b”： {<br> “完成百分比”： 0，<br> “状态”： “NEW”，<br> “名称”： “第一个任务”，<br> “ID”： “4c9a97db0000000f13ee4446b9aead9b”，<br> “任务编号”： 1 <br> }，<br> "4ca28ba600002024cd49e75bd43cf601"： {<br> "percentComplete"： 0，<br> "status"： "INP：A"，<br> "name"： "second task"，<br> "ID"： "4ca28ba600002024cd49e75bd43cf601"，<br> "taskNumber"： 2 <br> } <br> } <br>}</pre>
...
有关可能的字段引用的列表，请参阅[API资源管理器](../../wf-api/general/api-explorer.md)
...
<pre>/attask/api/&lt;受支持的版本&gt;/project/search？fields=tasks</pre>以下请求获取任务分配：
<pre>/attask/api/&lt;受支持的版本&gt;/task/search？fields=assignments</pre>

#### 搜索多个嵌套字段

默认情况下，仅返回每个任务的名称和ID，但可以使用冒号语法指定其他嵌套字段。 要查看相关对象或收藏集的所有可用字段，只需在对象/收藏集引用中附加冒号和星号即可。
<pre>/attask/api/&lt;supported-version&gt;/task/search？fields=assignments：*</pre>
...
<pre>/attask/api/&lt;supported-version&gt;/work/myWork</pre>命名查询支持请求字段参数以检索其他字段。 一些命名查询也接受其他过滤器。 有关对象中允许的命名查询列表，请参阅[API Explorer](https://developer.adobe.com/workfront/api-explorer/)中对象的“操作”选项卡。

#### 使用`Count`

您可以使用`count`返回与查询匹配的结果数。 当您不需要结果中的数据时，这可能很有用。 通过只返回计数，服务器可以更快地处理请求并节省带宽。 例如，请求
<pre>GET /attask/api/&lt;supported-version&gt;/project/count？status=CUR</pre>按以下格式返回结果数：
<pre>{<br> “计数”： 3 <br>}</pre>返回计数的数据传输比返回完整对象的数据传输小得多。 语法与search命令相同。

### 请求报告

您可以执行报表请求，其中只需要一个或多个分组的某些字段的汇总。 如以下示例所示，报表语法与SOAP API的语法相同：
<pre>GET /attask/api/&lt;supported-version&gt;/hour/report？project：name_1_GroupBy=true&amp;hours_AggFunc=sum</pre>将返回以下结果
...
<pre>GET /attask/api/&lt;supported-version&gt;/project/123abcxxxxxxxxxxxxxxxxxxxx？method=put &amp;updates={ accessRules： [ {accessorID： 'abc123'， accessorObjCode： 'USER'， coreAction： 'VIEW'} ] }</pre>或者，仅与新用户共享并保持现有权限不变：
<pre>GET /attask/api/&lt;supported-version&gt;/project/123abcxxxxxxxxxxxxxxxxxxxxxx/share？method=put&amp;accessorID=abc123&amp;accessorObjCode=USER&amp;coreAction=VIEW</pre>要检索现有的访问规则，请执行以下操作：
<pre>GET /attask/api/&lt;supported-version&gt;/project/123abcxxxxxxxxxxxxxxxxxxxxxxxx？fields=accessRules：*</pre>
...
以下是创建新项目的请求示例：
<pre>POST /attask/api/&lt;supported-version&gt;/project？name=新建项目</pre>响应包括新创建的项目及其新ID和任何其他指定字段。
...
PUT的响应与GET相同。 在这两种情况下，服务器都会在更新后返回对象的新状态。 所有用于更改对GET请求的响应的规则也可与PUT配合使用，例如指定要返回的其他字段、自定义数据等。

### 编辑对象

对象的更新始终使用对象的唯一URI通过ID完成。 要更新的字段被指定为请求参数。 例如，要更改项目的名称，您可以发送类似于以下内容的请求：
<pre>PUT /attask/api/&lt;supported-version&gt;/project/4c7...？name=新建项目名称<br>PUT /attask/api/&lt;supported-version&gt;/project？id=4c7...&amp;name=新建项目名称</pre>由于更新需要ID，因此如果服务器上不存在该对象，此操作将失败（不插入）。

### 指定JSON编辑

如以下示例所示，您可以使用更新请求参数指定要使用JSON语法更新的字段：
<pre>PUT /attask/api/&lt;supported-version&gt;/project/4c7...？updates= <br>{<br>名称： "New Project Name"， <br>状态： "CUR"， <br> ... <br>}</pre>

### 进行嵌套更新

某些对象具有可以更新的私有集合。 例如，以下示例演示了如何覆盖给定任务的现有分配：
<pre>PUT /attask/api/&lt;supported-version&gt;/task/4c7...？updates= <br>{<br>分配： [ <br> { <br> assignedToID： "2222...54d0， <br> assignmentPercent： 50.0 <br> }，{ <br> roleID： "1111...54d0"<br> } <br> ] <br>}</pre>

>[!NOTE]
>
>虽然对顶级所做的更新是稀疏的，但对集合或嵌套对象的更新将完全替换现有集合。 要在任务上编辑单个分配而不影响对象，请在分配上使用PUT，而不是在任务上使用PUT。
...
<pre>PUT /attask/api/&lt;supported-version&gt;/project/4c7...？action=calculateTimeline<br><br>或<br><br>PUT /attask/api/&lt;supported-version&gt;/project/4c7.../calculateTimeline </pre>

### 移动对象

下面演示了将任务从一个项目移动到另一个项目的语法：
<pre>PUT /attask/api/&lt;supported-version&gt;/task/4c7.../move？projectID=5d8...</pre>此处提供了每种操作类型的示例：(??)
<pre>PUT /attask/api/&lt;supported-version&gt;/project/1234/approveApproval<br><br>PUT /attask/api/&lt;supported-version&gt;/project/1234/calculateFinance<br><br>PUT /attask/api/&lt;supported-version&gt;/project/1234/calculateTimeline<br><br>PUT /attask/api/&lt;supported-version&gt;/project/12334/recall批准<br><br>PUT /attask/api/&lt;supported-version&gt;/project/1234/rejectApproval<br><br>PUT /attask/api/&lt;supported-version&gt;/task/1234/move<br><br>PUT /attask/api/&lt;supported-version&gt;/workitem/1234/markViewed<br><br></pre>只有移动操作需要标识附加属性，以指定要移动工作项的项目。
...
<pre>DELETE/attask/api/&lt;supported-version&gt;/task/4c78821c0000d6fa8d5e52f07a1d54d0 <br>DELETE/attask/api/&lt;supported-version&gt;/task？id=4c78821c0000d6fa8d5e52f07a1d54d0 <br>DELETE /attask/api/&lt;supported-version&gt;/task/4c78821c000d6fa8d5e52f07a1d54d0？force=true <br>DELETE/attask/api/&lt;supported-version&gt;/task？id=4c78821c0000d6fa8d5e52f07a1d54d0？force=true</pre>
