...
* 如果您对项目具有“查看财务数据”和“查看一般财务”权限，则只能查看“财务”部分中的字段。 您无法编辑此分区中的字段。
* 如果您对项目具有“编辑财务数据”和“编辑常规财务”权限，则可以更新此部分中的字段。

选择多个项目进行批量编辑时，存在以下情况：

* 如果选择至少一个具有“查看常规财务”（而不是“编辑常规财务”）权限的项目，则只能查看此分区中所有选定项目的字段。 不能批量编辑Finance节中的字段。
...
1. 单击左侧面板中的&#x200B;**财务**。
   ...
   项目编辑框中的![财务区域](assets/edit-project-finance-section.png)
...
您可以定义用户为项目获得的权限，以及用户与任意这些对象类型交互时项目中的任务和问题。

>[!IMPORTANT]
>
>在编辑项目时修改对项目、任务和问题的权限会确立在访问更改后用户与这些对象类型交互时如何授予权限。
>
>更改此配置不会以追溯方式修改用户对这些对象类型的权限，不会修改用户已分配或包含在其中的项目的权限。
