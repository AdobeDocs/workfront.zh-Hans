...
description: 您可以将项目从Microsoft项目导入Adobe Workfront，并在一个应用程序中管理所有项目。 每次从Microsoft项目导入项目时，都会在Workfront中创建一个新项目。
author: Alina
...
您可以将项目从Microsoft项目导入Adobe Workfront，并在一个应用程序中管理所有项目。 每次从Microsoft项目导入项目时，都会在Workfront中创建一个新项目。

>[!IMPORTANT]
>
>并非所有Microsoft项目字段都已传输到Workfront。
>
...
您可以从“主菜单”的“项目”区域，或从项目组合或项目群的“项目”区域创建项目。

1. 转到Microsoft项目，然后打开要从Workfront中导入的项目。
...
1. 单击 **选择文件**，然后在您的计算机上浏览从Microsoft项目导出的.xml文件。
1. 导入所选文件。

   Workfront会开始导入过程，并根据从Microsoft项目导出的文件创建新项目。

   导入过程完成后，系统会将您定向到新项目页面，该页面会显示有关导入已成功完成的确认信息。

   >[!NOTE]
   >
   >Workfront对文件上载有15分钟的时间限制。 如果文件上传所花费的时间超过此时间，我们建议您将项目划分为较小的项目，并单独导入这些项目。 将任务导入Workfront后，将任务从一个项目移动到另一个项目，以将它们合并到一个项目中。 有关移动任务的信息，请参阅 [移动任务](../../../manage-work/tasks/manage-tasks/move-tasks.md).

1. （可选）继续在Workfront中编辑项目。 有关编辑项目的信息，请参阅 [编辑项目](../../../manage-work/projects/manage-projects/edit-projects.md).

   从模板创建的新项目的状态与Workfront管理员在“项目首选项”区域或组项目首选项区域定义的状态相对应。 有关配置项目首选项的信息，请参阅 [配置系统范围的项目首选项](../../../administration-and-setup/set-up-workfront/configure-system-defaults/set-project-preferences.md).
