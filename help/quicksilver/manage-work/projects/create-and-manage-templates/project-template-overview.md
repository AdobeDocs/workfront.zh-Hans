...
* 决定您要将哪个文档存储用于未来的项目。 某些组织可以访问以下文档存储类型：

   * 旧版Workfront存储
   * Adobe 云存储

  添加文档会因您选择的存储类型而异。 您为模板选择的存储类型将影响未来项目将继承的存储类型。

  将模板添加到现有项目不会影响项目的存储类型。

  有关更多信息，另请参阅：

   * [创建项目模板](/help/quicksilver/manage-work/projects/create-and-manage-templates/create-template.md)
