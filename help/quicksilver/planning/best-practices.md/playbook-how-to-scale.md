---
title: 将您的首次成功转化为可持续的发展势头：管理扩展行动手册
description: 了解如何在首次成功实施后推广Adobe Workfront Planning。
...
## &#x200B;2. 核心理念：将Workfront Planning作为调节引擎
...
* [全球通道](#the-global-lanes)
* [当地的游乐场](#the-local-playgrounds-or-the-spokes)

有关信息，另请参阅：

* [推出您的战略主页：30天的启动台](/help/quicksilver/planning/best-practices.md/30-day-launchpad.md)
* [架构您的成功：建模您的营销活动层次结构](/help/quicksilver/planning/best-practices.md/model-campaign-hierarchy.md)

### 全球通道

以下是全局通道的特征：

* **受控对象**：每个团队必须用于企业报表的对象（例如，`Strategic Pillar`、`Region`、`Fiscal Quarter`）。
...
* [调整优先级：数据与可视化图表](#align-priorities-data-vs-visualization)
* [服务引导的观察阶段](#the-service-led-observation-phase)
* [追溯协调](#retroactive-harmonization)
...
* **确定可见性最小值**：与领导团队合作，确定企业报表中必须清除的3-5个字段（例如，`Strategic Alignment`、`Start Date`、`Budget`）。 仅将您的执行精力集中在可见性最小字段上。
...
2. 重命名要弃用的字段：重命名前缀为`[DEPRECATED]`或`z_`的本地字段（例如，`z_Language (Old)`）。 这会将字段推到字段选取器的底部，并告知用户它不应再是真实来源。
...
* [“必要的混乱”发现研讨会](#the-necessary-mess-discovery-workshop)
* [“战略可视性”协调会议](#the-strategic-visibility-alignment-session)
* [“与全球对话”的展示](#the-spoke-to-global-showcase)
* [“持续支持”办公时间](#the-ongoing-support-office-hours)
