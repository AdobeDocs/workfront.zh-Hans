---
title: 推出您的战略主页：30天的启动台
description: 了解如何使用优先考虑心理安全、尽量减少干扰并快速取得好处的策略来推出Workfront Planning。
...
本指南适用于实施Workfront Planning的Workfront管理员。
...
要成功推出产品，明智的做法是使用引导式自主权。 这意味着，作为管理员，您可以定义团队如何在由控制的默认值和模板组成的结构化系统中工作。

您可以定义道路上的车道（或管辖的默认值和模板），同时允许团队灵活地选择其内部的工作路径。

最重要的是，引导式自治应该是进化式的。 您无需在第一天实施完整的企业分类，而是可以根据第一个试点小组的需求从最低限度的全局定义开始。

随着更多团队的加入，您可以观察常见的模式并对全球标准进行迭代。
...
从一开始就建立中央集权的结构，奠定战略基础。

虽然尚不需要复杂的企业级分类，但您应立即创建两个环境。 例如，当您采用轴辐式体系结构方法时，可以创建以下内容：

* **全局分类工作区**（中心）：这应该是您系统的常规结构和命名法。
* 前&#x200B;**团队工作区** （分支）：这应该与单个团队的工作方式相匹配。
...
* **连接策略以正常工作：**&#x200B;启用规划记录与Workfront对象之间的链接。

  有关信息，请参阅[连接的记录类型概述](/help/quicksilver/planning/architecture/connect-record-types-overview.md)。

* **镜像当前数据：**&#x200B;使用团队的现有工具（例如，Excel或其他核心应用程序）来填充Workfront Planning，确保数据让人感到熟悉和相关。

* **专为灵活性而打造：**&#x200B;请记住，Workfront Planning旨在改进。 您可以从团队级流程开始，在业务准备就绪时轻松提升或迁移这些流程到企业级管理。

### 第15天：第一个突破时刻

我们建议你以第15天前取得第一次重大胜利为目标。 这应该是自动项目创建。

要实现此目的，您必须实施以下内容：

1. 配置本地自动化以标识“执行就绪”状态。

2. 在Workfront中触发链接项目的创建。

有关信息，请参阅[配置Adobe Workfront计划自动化](/help/quicksilver/planning/records/configure-automations-to-create-records.md)。

这可确保您的团队看到其战略意图会自动触发执行工作。 这一突破时刻证明了战略与工作之间桥梁的价值，并构建了认同感。

### 第16-30天：逐步过渡到企业规模

获得闪电时刻后，您可以将团队的起点移至Workfront规划中。

随着此团队优先模型的成功，您可以通过确定有效的模式来开始企业扩展之路。

要从团队优先扩展到企业规模，请考虑以下事项：

* **引入引入表单：**&#x200B;使用请求表单作为策略请求的新网关，以确保数据完整性。

  有关信息，请参阅[在Adobe Workfront Planning中创建和管理申请表单](/help/quicksilver/planning/requests/create-request-form.md)。

* **观察并标准化：**&#x200B;使用全局分类工作区来观察试验小组如何使用工具。 这些现实世界的示例将塑造您最终的企业设计。

* **停用旧版电子表格：**&#x200B;随着流程的成熟完全移至Workfront Planning中。

* **Shift超级用户：**&#x200B;使用记录表视图进行直接、高速的计划管理。

  有关信息，请参阅[管理表视图](/help/quicksilver/planning/views/manage-the-table-view.md)。

## 快速启动板的成功量度概述

我们建议您不断查找成功标记并根据您的发现进行调整。

寻找这些早期健康指标：

* **采用：**&#x200B;前5到10个战略记录已成功链接到执行项目，并且创建记录和自动创建链接项目的过程是无缝的，而且可以轻松重复。

* **宣传：**&#x200B;试点团队开始向相邻团队宣传该工具。

* **效率：**&#x200B;自动化缩短了从战略批准到项目开始的时间。

* **信任：**&#x200B;利益相关者使用Workfront Planning时间表而不是手动状态平台进行审核。

## 最佳实践和提示

实施的成功取决于您使用的方法以及您从一开始就设定的期望。

以下是有关如何成功实施的一些建议。

### 待办事项：

* **从兴奋的试点团队开始：**&#x200B;确认全球推出是一项显着的提升。 专注于概念验证，团队成员渴望创新并能为他人提供蓝图。

* **定义通道，而不是墙：**&#x200B;使用引导式自主功能提供结构，但不阻止探索。

* **优先考虑心理安全：**&#x200B;让团队知道，在熟悉新工作流程之前，旧工具保持不变。

* **庆祝快速成功：**&#x200B;向整个团队重点介绍第一个自动项目创建。

* **建立冠军：**&#x200B;确定每个团队中的超级用户以领导过渡。

### 不要：

* **不强制进行大规模更改：**&#x200B;强制所有人在第一天进行切换会导致技术方面的指责。

* **不要忽略原因：**&#x200B;确保每个团队成员都了解战略线程如何使其工作更显眼。

* **不要等待完善：**&#x200B;您的分类将不断改进。 先从最好的猜测开始，然后一步一步完善。
