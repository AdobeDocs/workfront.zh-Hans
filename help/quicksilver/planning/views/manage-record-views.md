...
在Adobe Workfront Planning区域中选择记录类型后，您可以通过各种方式显示该类型的所有记录。

您可以查看多种格式的记录，这让您能够灵活地以最适合自己的方式探索和了解信息。 无论您是要结构化概览、按时间顺序排列文章、基于日期的布局还是要简单的可滚动列表，每个视图都提供了独特的透视。
...
* 列表
  ...
  您可以在列表视图的“连接的记录”页中查看记录。

  >[!IMPORTANT]
  >
  >您不能将列表视图应用于记录类型页面上的记录列表。 您只能将记录的“已连接”记录页中的列表视图应用于已连接项目的列表。<!--this will change-->

  有关更多信息，请参阅以下文章：

   * [将“连接的记录”页面添加到记录](/help/quicksilver/planning/records/add-a-connected-records-page-to-a-record.md)
   * [管理列表视图](/help/quicksilver/planning/views/manage-the-list-view.md)
...
有关列表视图的信息，请参阅[管理列表视图](/help/quicksilver/planning/views/manage-the-list-view.md)。
