---
title: 在报表中查看Workfront规划信息
description: 使用“画布功能板”时，您可以在报表中查看Adobe Workfront Planning信息。
author: Alina
feature: Workfront Planning
role: User, Admin
recommendations: noDisplay, noCatalog
source-git-commit: 8b161669c427c316ba28b814dfa7a6d095cd4309
workflow-type: tm+mt
source-wordcount: '206'
ht-degree: 6%

---


# 在报表中查看Workfront规划信息

使用“画布功能板”时，您可以在报表中查看Adobe Workfront Planning信息。

>[!IMPORTANT]
>
>画布功能板目前仅适用于参与Beta测试阶段的用户。在此阶段，部分功能可能无法完成或无法按预期工作。
>以下云提供商未提供测试版：
>
>* 自带Amazon Web Services密钥
>* Azure
>* Google Cloud平台> 有关详细信息，请参阅[管理画布功能板：文章索引](/help/quicksilver/reports-and-dashboards/canvas-dashboards/manage-canvas-dashboards/manage-canvas-dashboards.md)。

## 访问权限要求

+++ 展开可查看本文所述功能的访问权限要求。 

<table style="table-layout:auto"> 
<col> 
</col> 
<col> 
</col> 
<tbody> 
<tr> 
   <td role="rowheader"><p>Adobe Workfront包</p></td> 
   <td> 
<p>任何Workfront或工作流包</p>
<p>任何规划包</p>
   </td> </tr>

</tr> 
  <tr> 
   <td role="rowheader"><p>Adobe Workfront许可证</p></td> 
   <td><p>标准</p> 
  </td> 
  </tr>
   <tr> 
   <td role="rowheader"><p>访问级别配置</p></td> 
   <td><p>编辑对报告、功能板和日历的访问权限</p>
  </td> 
  </tr>  
  <tr> 
   <td role="rowheader"><p>对象权限</p></td> 
   <td><p>查看对工作区的权限</a> </p>  
   <p>系统管理员对所有工作区具有权限，包括他们未创建的工作区</p>  
   </td> 
  </tr>  
</tbody> 
</table>

有关Workfront访问要求的详细信息，请参阅Workfront文档中的[访问要求](/help/quicksilver/administration-and-setup/add-users/access-levels-and-object-permissions/access-level-requirements-in-documentation.md)。

+++

## 在画布功能板中查看计划信息

如果您正在参与“画布功能板测试版”，则可以构建引用Planning信息（记录类型、记录和字段）的报表。

有关更多信息，请参阅以下文章：

* [在画布功能板中构建表报告](/help/quicksilver/reports-and-dashboards/canvas-dashboards/add-reports/build-table-report.md)
* [在画布功能板中构建KPI报告](/help/quicksilver/reports-and-dashboards/canvas-dashboards/add-reports/build-kpi-report.md)
* [在画布功能板中构建KPI报告](/help/quicksilver/reports-and-dashboards/canvas-dashboards/add-reports/build-kpi-report.md)
* [在画布功能板中使用货币字段](/help/quicksilver/reports-and-dashboards/canvas-dashboards/manage-canvas-dashboards/switch-currencies.md)
