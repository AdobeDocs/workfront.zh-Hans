...
有关每个操作的更多详细信息和示例，请参见 [Workfront Planning API开发人员文档](https://developer.adobe.com/wf-planning/).
...
默认情况下，Planning API请求从列表开头返回500个结果。 要覆盖结果数的默认限制，您可以使用 `limit` 参数，并将其设置为其他数字，最多可达2000个结果。

我们建议您考虑通过添加 `offset` 参数到您的请求。 分页响应允许您指定应返回的第一个结果的位置。
...
有关排序的详细信息，请参阅 [在API中对查询结果进行排序](#sorting-query-results-in-the-api) 本文章中。
