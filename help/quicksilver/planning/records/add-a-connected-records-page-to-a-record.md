...
<td> 
   ...
   <ul><li><p>Adobe Experience Manager许可证以及Adobe Experience Manager与Workfront之间的集成，用于连接AEM对象与Planning记录类型。</p>
...
<tr> 
  ...
  <tr>
   ...
   <td>
   <p>向工作区提供权限或更高的权限、记录类型和管理记录的权限 </p>  
...
1. （可选且有条件）如果要为显示构建页面的记录或对象类型有多个已连接字段，请从&#x200B;**选择引用字段**&#x200B;列表中单击要在已连接记录页面中显示其记录或对象的字段。
   ...
   * 项目对象类型的列表视图
   ...
   默认情况下，将显示连接的记录的前五个字段。 默认情况下，不显示查找字段。
