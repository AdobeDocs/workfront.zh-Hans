...
* 任何用户都不能对辅助工作区中的全局记录类型执行以下操作：
  ...
  * 创建和管理请求表单
    ...
    <span class="preview">您可以使用辅助记录类型页面中的请求表单为辅助记录类型创建记录，或者在填写为主要全局记录类型创建的请求表单并更新Workspace字段时，通过明确指示应将记录保存到的工作区。
    有关信息，请参阅[提交Adobe Workfront Planning请求以创建记录](/help/quicksilver/planning/requests/submit-requests.md)。</span>
...
1. 将记录类型配置为可在特定工作区中连接。
   ...
   >[!TIP]
   >
   >只有系统管理员才能将记录类型指定为可从系统中的所有工作区连接。 所有其他工作区管理员都可以指定要从中连接到记录类型的特定工作区。
   >
   >有关信息，请参阅[为记录类型配置跨工作区功能](/help/quicksilver/planning/architecture/configure-record-type-cross-workspace-capabilities.md)。
