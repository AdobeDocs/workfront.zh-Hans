...
本文概述记录类型如何连接，并说明您可以在记录与对象类型之间建立连接的类型。
...
* 将记录类型与另一个记录类型或另一个应用程序中的对象类型连接后，将出现以下情况：
   ...
   * **当您添加来自您连接到的记录或对象的链接（或查找）字段时**：包含来自您连接到的记录的信息的链接（或查找）字段将显示在您连接到的记录上。
...
* 查找字段前面有一个图标，用于指示字段中显示的信息类型。

## 连接类型

在两个记录类型之间或在记录与来自另一个应用程序的对象类型之间建立连接后，可以在连接的记录字段中添加记录。

根据可以向连接中添加的记录数量，在连接记录类型时可以选择以下连接类型：
