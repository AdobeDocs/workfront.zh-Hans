...
在工作区中连接记录类型后，您可以创建一个层次结构来组织这些连接。

层级将记录和对象类型组织为父子关系，每个层级最多可包含4个对象类型。 在一个工作区中最多可创建5个层次结构。
...
1. （可选）单击层次结构中最后一个记录类型的名称，然后单击该记录类型视图中的记录名称。 这将打开记录的详细信息页面。 在记录页面顶部的记录痕迹导航中查找您创建的层次结构。
