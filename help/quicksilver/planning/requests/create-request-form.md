...
1. 在&#x200B;**创建请求表单**&#x200B;框中，更新请求表单的名称。 默认情况下，表单的名称为&#x200B;**无标题表单**。<!--check this; you logged a bug to rename it to 'Untitled request form' but was it fixed?-->
1. （可选）为请求表单添加&#x200B;**描述**。
...
1. 单击&#x200B;**创建**。
   ...
   * **默认分节**：这是Workfront应用于请求表单的默认分节符。 所有记录字段都显示在&#x200B;**默认部分**&#x200B;区域。
...
1. （可选）单击表单左侧的&#x200B;**内容元素**&#x200B;选项卡，然后添加以下任意元素：
   ...
   * **分区界限**：这是表单上包含多个字段的区域。
     ...
     >[!TIP]
     >
     >构建Planning请求表单与构建Workfront自定义表单类似。 有关详细信息，请参阅[创建自定义表单](/help/quicksilver/administration-and-setup/customize-workfront/create-manage-custom-forms/form-designer/design-a-form/design-a-form.md)。

1. （可选）单击&#x200B;**预览**&#x200B;以查看当其他用户使用表单提交请求时，该表单将如何显示给其他用户。
1. 继续执行以下操作之一：

   * [如果要为生产环境中的表单配置更多详细信息，请配置表单设置](#configure-form-settings)
   * [发布表单](#publish-form)（如果不想进一步配置设置）。
...
1. 从以下选项中选择：
   ...
   * **请求在创建请求对象时完成**：这将在创建记录时完成请求。
   * **请求在请求的对象完成时完成**：当记录标记为完成时，这将完成请求。
...
1. 创建并保存表单后，单击&#x200B;**发布**&#x200B;以发布表单并获取其唯一链接。
...
1. 转到请求表单列表，然后执行下列操作之一：

   * 单击记录类型页面上请求表单名称右侧的&#x200B;**更多**&#x200B;菜单![更多菜单](assets/more-menu.png)。
   * 单击列表中的请求表单的名称以将其打开。
...
1. （视情况而定）如果打开了表单，请单击&#x200B;**表单**&#x200B;选项卡右下角的&#x200B;**保存**&#x200B;以保存表单。
