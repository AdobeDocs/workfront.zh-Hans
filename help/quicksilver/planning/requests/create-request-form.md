...
1. 单击任意字段，然后使用表单右侧面板中的控件定义其大小或以下任何信息：
   ...
   * **添加逻辑**：定义必须满足哪些条件才能显示或隐藏字段。
   ...
   选择表单上的字段后，每个字段的字段类型都会显示在右侧面板的顶部。 请求表单上字段类型的名称与它们在表视图中的名称不同。 下表描述了表视图中字段类型的名称与记录类型请求表单中相同字段的名称之间的差异：

   | Workfront Planning字段类型 | 请求表单字段类型 |
   |-------------------------------|-------------------------|
   | 单行文本 | 单行文本 |
   | 段落 | 带格式文本 |
   | 货币、数字、百分比 | 单行文本 |
   ...
   >[!NOTE]
   >
   >   字段的格式将保留。 例如，虽然货币和百分比显示为单行文本字段，但字段中的信息显示为货币和百分比值。
