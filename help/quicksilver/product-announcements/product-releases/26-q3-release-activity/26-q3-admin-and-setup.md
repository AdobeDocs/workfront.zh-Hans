...
* 我们已将“访问级别”框中的“许可证类型”字段重命名为“工作流许可证类型”。 此重新标记的功能没有变化。\
  有关信息，[请参阅创建和修改自定义访问级别](/help/quicksilver/administration-and-setup/add-users/configure-and-grant-access/create-modify-access-levels.md)。
...
已购买相同数量的Workflow和Planning许可证的客户具有以下可用许可证类型：
  ...
  >[!NOTE]
  >
  >您可以在Workflow和Planning之间为用户分配混合许可证组合，但Planning许可证类型不能高于Workflow许可证类型。
  >
