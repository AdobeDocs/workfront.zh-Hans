...
description: 本页介绍了2018.2 Beta 5版的“预览”环境中最近提供的所有更改。 该功能将于2018年6月1日在“预览”环境中可用。 Beta 5版本的校对增强功能将于6月4日星期一在“预览”环境中发布。 该版本将于2018年7月在生产环境中提供。
author: Luke
...
本页介绍了2018.2 Beta 5版的“预览”环境中最近提供的所有更改。 该功能将于2018年6月1日在“预览”环境中可用。 Beta 5版本的校对增强功能将于6月4日星期一在“预览”环境中发布。 该版本将于2018年7月在生产环境中提供。
...
2018.2 Beta 5版本包含面向Workfront管理员和其他用户的增强功能：
...
如果在Outlook中使用日历，则可以集成日历，以在“主页日历”视图中显示您的Outlook事件。

有关更多信息，请参阅 [主页日历视图](../../../../workfront-basics/using-home/using-the-home-area/home-calendar-view.md).

## 主页工作列表（左侧面板）的其他更新 {#additional-updates-to-the-work-list-left-panel-in-home}

以下增强功能现在可用于“主页”区域中的“工作列表”：

* 现在，“过滤器”下拉菜单中的“已完成”选项旁的括号中显示了已完成项目的数量。
...
* 系统会显示前2周已完成的项目。
  ...
  有关在“主页”区域查看已完成工作的信息，请参阅 [在主页区域的工作列表中显示项目](../../../../workfront-basics/using-home/using-the-home-area/display-items-in-home-work-list.md) 在文章中 [在主页区域的工作列表中显示项目](../../../../workfront-basics/using-home/using-the-home-area/display-items-in-home-work-list.md).

* 添加持续时间字段和分配字段，以在主页区域中选择项目时显示。

  以前，“工作总揽”字段默认可用；但是，如果将其删除，则无法重新添加。 “持续时间”字段以前不可添加。
...
>[!NOTE]
>
在23.1版本中，资源计划工具已被弃用并从Workfront中删除。 有关使用工作负载均衡器调度资源的信息，请参见 [工作负载均衡器概述](../../../../resource-mgmt/workload-balancer/overview-workload-balancer.md).

在自动资源计划设置中，您现在可以为工作角色分配限制。 这样，您就可以使用相同的角色控制被分配工作的资源数量。

以前，您无法指定可以为某个工作角色中的多少个用户分配工作。

有关更多信息，请参阅“在计划区域中手动分配未分配的任务和问题”。

## 资源规划者中的项目和角色视图改进 {#project-and-role-view-improvements-in-the-resource-planner}

资源规划者的“项目”和“角色”视图现在包含以下改进：
...
* 直接从资源规划者快速访问项目和用户。
* “项目”视图中更快的拖放功能，可排定项目的优先级。

在这些改进之前，您已报告资源规划者加载缓慢，并且您注意到显示的数据中存在不一致的情况。 有了这些改进，这些应该被消除。

有关信息以及要了解资源规划者的新限制，请参阅 [资源规划者显示限制](../../../../resource-mgmt/resource-planning/resource-planner-display-limitations.md)
...
由于我们一直在努力改进列表的功能，因此暂时禁用了调整以下项目列表中的列宽度的功能：
...
现在，当您拖动列的右边框以调整其大小时，右侧的相邻列会保留其大小，从而使列表变宽，而不是同时进行修改。 此外，您还可以将列的边框拖动到相邻列的边框右上方。

在进行此项改进之前，已调整大小的列右侧的相邻列也会按比例调整大小以适合屏幕，并且您不能将列的边框拖过相邻列的右边框。  

有关调整列表中列重新排序大小的信息，请参见 [修改列宽和顺序](../../../../reports-and-dashboards/reports/reporting-elements/modify-column-width-order.md).

要参与我们的Beta测试计划以获得当前的列表改进，请参阅 [新列表研究。](http://community.workfront.com/discussions/community-home/digestviewer/viewthread?GroupId=457&amp;MessageKey=6b135c15-33dd-4fa2-8bc3-7cd7f7740c57&amp;CommunityKey=0425cafc-f0ec-47fc-be20-a21dc073d520&amp;tab=digestviewer&amp;ReturnUrl=%2fdiscussions%2fcommunity-home%2fdigestviewer%3fCommunityKey%3d0425cafc-f0ec-47fc-be20-a21dc073d520) （需要登录）
...
我们将向列表或报表中的文档视图添加一个名为大型缩略图的新字段。 在文档视图中选择此字段时，会在列表或报告中显示文档的400像素宽缩略图。
...
快速过滤器的用途是帮助您直接导航到大型列表中对您很重要的那些项目，以便您能够快速查看、更新这些项目或与其他人共享这些项目。

目前，快速过滤器仅适用于以下子选项卡中的项目列表：
...
您现在可以在项目或任务查看和筛选器中引用问题集合。 您只能在构建报表时使用文本模式执行此操作。
...
有关使用文本模式的信息，请参阅  [文本模式的常见用法概述](../../../../reports-and-dashboards/reports/text-mode/understand-common-uses-text-mode.md).
...
现在，向Workfront中的文档添加新版本时，“新版本”菜单包含其他选项，并且现在Workfront中您可以添加新版本的区域是一致的。
...
在此版本发布当天后不久，将在Android测试版移动应用程序上提供以下改进：
...
* 查看项目的“详细信息”选项卡时的新外观
  ...
  在移动设备应用程序的Android Beta版本中查看项目时，界面已发生更改，以便更轻松地编辑、完成项目或向项目附加文档。

* 记录时间的新体验

  记录时间比以往更快、更简单，记录时间按钮更易于访问，记录小时数界面更简化。

在此版本中，这些改进仅适用于Workfront移动设备应用程序的Android测试版。 它们当前不可用于iOS。

有关如何注册为测试版并下载Android Beta版本的Workfront移动设备应用程序的更多信息，请参阅。
...
* [在桌面校对查看器中查看Workfront文档审批](#view-workfront-document-approvals-in-the-desktop-proofing-viewer)
* [配置打开新选项卡和窗口的链接，以便在桌面校对查看器中打开](#configure-links-that-open-new-tabs-and-windows-to-open-within-the-desktop-proofing-viewer)
* [验证查看器中的存在指示器](#presence-indicator-in-the-proofing-viewer)
...
“Print Summary”（打印摘要）页面已更新，具有新外观和改进功能。
...
以前，您无法将个人用户添加到验证中。 相反，您只能复制公共URL或嵌入代码。
...
您现在可以将标记工具配置为始终显示，而不是显示在必须打开的菜单中。 这使得在工具之间切换更加快速。 通过这种方式配置后，标记工具水平显示在Web验证查看器和桌面验证查看器的顶部。
...
现在，当您更改验证中评论列表中的排序选项时，该选项会在您下次在Web验证查看器或桌面验证查看器中打开任何验证时成为默认排序选项。 

有关更多信息，请参阅文章中的。

### 在桌面校对查看器中查看Workfront文档审批 {#view-workfront-document-approvals-in-the-desktop-proofing-viewer}

现在，您可以在桌面验证查看器中做出Workfront文档批准决策。

以前，只有Web验证查看者允许您作出Workfront文档审批决定。 
...
在桌面校对查看器中校对交互式内容时，您现在可以配置在新选项卡或新窗口中打开的链接，以便在桌面校对查看器中打开，以便您可以继续校对。
...
在桌面验证查看器中查看交互式验证中的URL时，您现在可以筛选注释以显示仅在当前页面上所做的注释。 
...
您现在可以向系统中添加任何自定义设备，从而允许用户在桌面验证查看器中查看验证时，查看交互式内容并模拟其在特定设备上的显示方式。
...
现在，您可以在Workfront中生成校对链接并直接从Workfront共享它。 或者，您可以复制URL并使用替代方法分发该URL。

在此更改之前，您只能复制Workfront中的验证链接，并使用替代方法分发它。
...
>[!NOTE]
>
当前可用的嵌入链接将在未来版本中删除。 嵌入链接仍可以通过Workfront API访问。

### 报告Workfront中的其他验证数据 {#report-on-additional-proofing-data-in-workfront}

在包含文档版本对象的报告中（例如文档版本报告和验证审批报告），有多个字段现在可供您查看其他验证信息。

* 校对截止日期

  显示验证截止日期中一周中的某天、日期、时间和年份。
...
在验证审批报告中，您可以添加字段，以查看不再有效的验证的验证审批决策。 您可通过将“批准者决策”字段添加到报表来执行此操作。
