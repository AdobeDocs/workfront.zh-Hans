...
将甘特图导出到PDF时，现在可以选择是否同时导出图表的图例以及图表本身。 图例中包含的项目只是您已启用以便在UI的甘特图中显示的选项。 如果项目任务中存在这些选项，则它们会包含在图例中。 例如，如果启用“甘特图”中的“里程碑”显示功能，则图例也会显示它们，但前提是至少有一个任务与里程碑关联。

在此更改之前，无法从导出的PDF中排除图例，并且该图例包含甘特图的所有可能选项和标记，无论这些选项和标记在UI中是启用还是存在。
