...
title: 数据存储模块
description: An [!DNL Adobe Workfront Fusion] 数据存储（与数据库或简单表类似）可以存储场景中的数据，从而可以在各个场景或场景运行之间传输数据。 在同步过程中，您可以使用数据存储来存储来自不同系统的新数据。
author: Becky
...
An [!DNL Adobe Workfront Fusion] 数据存储（与数据库或简单表类似）可以存储场景中的数据，从而可以在各个场景或场景运行之间传输数据。 在同步过程中，您可以使用数据存储来存储来自不同系统的新数据。

通过数据存储模块，您可以添加、替换、更新、检索、删除、搜索或计算您的记录数。 [!DNL Adobe Workfront Fusion] 数据存储。
...
使用 [!UICONTROL 数据存储] 模块时，必须首先创建一个数据存储。

有关创建数据存储的信息，请参阅 [数据存储位置 [!UICONTROL Adobe Workfront Fusion]](../../workfront-fusion/modules/data-stores.md)

## [!UICONTROL 数据存储] 模块及其字段

配置数据存储模块时， [!DNL Workfront Fusion] 显示下面列出的字段。 除此之外，还可能会显示其他数据存储字段，具体取决于诸如应用程序或服务中的访问级别等因素。 模块中的粗体标题表示必填字段。
...
>[!NOTE]
>
>当您尝试添加已存在于数据存储中的记录时，模块会引发错误，该记录会使用相同的名称和 [!UICONTROL 覆盖现有记录] 选项已禁用。
...
<table style="table-layout:auto">
 ...
 <tbody> 
  ...
  <tr> 
   <td>[！UICONTROL键] </td> 
   <td> <p>输入您希望模块添加或替换的记录的唯一键。 该键稍后可用于检索记录。 如果将此字段留空，将自动生成一个键。</p> </td> 
  </tr> 
  <tr> 
   <td>[！UICONTROL覆盖现有记录] </td> 
   <td> <p>启用此选项以覆盖记录。 您必须在上面的“键值”字段中指定要覆盖的记录。</p> </td> 
  </tr> 
  <tr> 
   <td>[！UICONTROL记录] </td> 
...
<table style="table-layout:auto">
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p style="font-weight: normal;">对于要作为排序依据的每个字段，请填写以下字段：</p> <p style="font-weight: bold;">[！UICONTROL键]</p> <p>选择要作为结果排序依据的列名。</p> <p style="font-weight: bold;">[！UICONTROL顺序]</p> <p>选择您要按升序还是降序对结果进行排序。</p> </td> 
  </tr> 
  <tr> 
   <td>[！UICONTROL限制]</td> 
