...
description: 为了做一个 [!DNL Adobe Workfront Fusion] 向需要OAuth 2.0授权的服务器发出HTTP(S)请求，您首先需要创建OAuth连接。 [!DNL Adobe Workfront Fusion] 确保通过此连接进行的所有调用都具有适当的授权标头，并在需要时自动刷新关联的令牌。
author: Becky
...
为了做一个 [!DNL Adobe Workfront Fusion] 向需要OAuth 2.0授权的服务器发出HTTP(S)请求，您首先需要创建OAuth连接。 [!DNL Adobe Workfront Fusion] 确保通过此连接进行的所有调用都具有适当的授权标头，并在需要时自动刷新关联的令牌。
...
其他流（如资源所有者密码凭据流和客户端凭据流）不自动通过此模块支持。
...
* [有关在HTTP >创建OAuth 2.0请求模块中创建连接的常规说明](#general-instructions-for-creating-a-connection-in-the-http--make-an-oauth-20-request-module)
...
1. 查找 `[!UICONTROL Authorize URI]` 和 `[!UICONTROL Token URI]` 在给定服务的API文档中。 这些是URL地址，通过这些地址， [!DNL Workfront Fusion] 与 [!DNL target] 服务。 这些地址用于OAuth授权。
...
1. （视情况而定）如果目标服务使用作用域（访问权限），请检查服务如何分隔各个作用域，并确保在高级设置中相应地设置分隔符。 如果未正确设置分隔符， [!DNL Workfront Fusion] 无法创建连接，您收到无效的范围错误。
1. 完成上述步骤后，您可以开始在中创建OAuth连接 [!DNL Workfront Fusion]. 将OAuth 2.0 HTTP(S)请求和响应处理模块添加到您的方案中。
   ...
   <table style="table-layout:auto">  
    ...
    <tbody> 
     ...
     <tr> 
      ...
      <td> <p>选择获取令牌的流程。</p> 
       <ul> 
        <li><strong>[！UICONTROL授权代码]</strong>：输入 <code>[!UICONTROL Authorize URI]</code> 和 <code>[!UICONTROL Token URI]</code> 来自服务的API文档。</li> 
        <li><strong>[！UICONTROL隐式]</strong>：输入 <code>[!UICONTROL Authorize URI]</code> 来自服务的API文档。</li> 
       </ul> </td> 
     </tr> 
     <tr> 
     ...
     <tr> 
      ...
      <td> <p>选择上面输入的范围应该用分隔符分隔的内容。 您可以在给定服务的开发人员(API)文档中找到此信息。</p> <p>警告：如果分隔符设置不正确， [!DNL Workfront Fusion] 无法创建连接，您收到无效的作用域错误。</p> </td> 
     </tr> 
     <tr> 
     ...
     <tr> 
      <td role="rowheader"> <p>[！UICONTROL Authorize参数]</p> </td> 
      <td> <p>添加要包含在授权调用中的任何参数。 以下标准参数始终自动包含在内，无需添加。</p> <p>标准参数：</p> 
       <ul> 
        <li> <p><strong>[！UICONTROL response_type]</strong> </p> <p> <code>code </code>（适用于[！UICONTROL授权代码流]和） <code>token </code>对于[！UICONTROL隐式流]</p> </li> 
     ...
     <tr> 
      ...
      <td> <p>指定要包含在[！UICONTROL令牌]和R[！UICONTROL刷新令牌]步骤的标头中的任何其他键和值。</p> <p>注释:  <p>OAuth 2.0标准在此步骤中支持至少2种客户端身份验证方法(<code>[!UICONTROL client_secret_basic]</code> 和 <code>[!UICONTROL client_secret_post]</code>)。 [!DNL Workfront Fusion] 不会自动支持 <code>[!UICONTROL client_secret_basic]</code> 方法。 如果您要连接的服务要求将客户端ID和客户端密钥组合为一个字符串，然后将base64编码到授权标头，则您应在此处添加该标头和密钥值。</p> <p> 有关OAuth 2.0身份验证的更多信息，请参阅 <a href="https://tools.ietf.org/html/rfc6749">OAuth 2.0授权框架</a>.</p> </p> </td> 
     </tr> 
     <tr> 
      <td role="rowheader"> <p>[！UICONTROL令牌放置]</p> </td> 
      <td> <p>选择在连接到指定的URL时是在[！UICONTROL标头]、[！UICONTROL查询字符串]中发送令牌，还是同时在这两者中发送。</p> <p>令牌最常在请求标头中发送。</p> </td> 
     </tr> 
     <tr> 
...
1. 继续访问 [OAuth 2.0请求模块设置](#oauth-20-request-module-setup).
...
1. 确保已创建项目、配置OAuth设置并生成凭据，如中所述 [Connect [!DNL Adobe Workfront Fusion] 到 [!DNL Google Services] 使用自定义OAuth客户端](../../../workfront-fusion/connections/connect-fusion-to-google-using-oauth.md).
...
1. 输入以下值：
   ...
   <table style="table-layout:auto">  
    ...
    <tbody> 
     ...
     <tr> 
      <td role="rowheader">[！UICONTROL作用域] </td> 
      <td> <p>添加单个范围。 有关作用域的详细信息，请参见 <a href="https://developers.google.com/identity/protocols/oauth2/scopes">OAuth 2.O作用域 [!DNL Google] API</a> 在 [!DNL Google] 文档。</p> </td> 
     </tr> 
     <tr> 
      <td role="rowheader">[！UICONTROL范围分隔符] </td> 
...
当您建立 [!DNL Oauth 2].0连接，如中所述 [为创建连接 [!DNL OAuth] 请求](#creating-a-connection-for-an-oauth-request)，根据需要继续设置模块。 所有授权令牌都会自动包含在此请求中，以及使用同一连接的任何其他请求中。
...
<table style="table-layout:auto">  
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>HTTP正文是在HTTP事务消息中传输的数据字节，紧跟在标头之后（如果有任何要使用的标头）。</p> 
    <ul> 
     ...
     <li> <p><strong>[！UICONTROL应用程序/x-www-form-urlencoded]</strong> </p> <p>此正文类型使用以下方式POST数据 <code>[!UICONTROL application/x-www-form-urlencoded]</code>.</p> <p>对象 <code>[!UICONTROL application/x-www-form-urlencoded]</code>，发送到服务器的HTTP消息正文本质上是一个查询字符串。 键和值在键值对中进行编码，分隔方式为 <code>&amp;</code> 和 <code>=</code> 键和值之间。 </p> <p>对于二进制数据， <code>use [!UICONTROL multipart/form-data]</code> 而是。</p> 
  ...
  <tr> 
   ...
   <td> <p>以秒为单位输入请求超时(1-300)。 默认值为40秒。</p> </td> 
  </tr> 
  <tr> 
