...
description: Adobe Workfront Fusion HTTP &gt；发出请求模块是一个通用模块，可用于配置HTTP请求并将其提交到服务器。 接收的HTTP响应随后包含在输出包中。
author: Becky
...
此 [!DNL Adobe Workfront Fusion] [!UICONTROL HTTP] > [!UICONTROL 创建请求模块] 是一个通用模块，可用于配置HTTP请求并将其提交到服务器。 接收的HTTP响应随后包含在输出包中。
...
有关的信息 [!DNL Adobe Workfront Fusion] 许可证，请参阅 [Adobe Workfront Fusion许可证](../../../workfront-fusion/get-started/license-automation-vs-integration.md)
  ...
  <tr> 
   ...
   <td> <p>HTTP正文是在HTTP事务消息中传输的数据字节，这些字节紧跟在标头之后（如果存在任何要使用的标头）。</p> 
    <ul> 
     ...
     <li> <p><strong>[！UICONTROL应用程序/x-www-form-urlencoded]</strong> </p> <p>此正文类型使用对[！UICONTROL]数据的POST <code>[!UICONTROL application/x-www-form-urlencoded]</code>.</p> <p>对象 <code>application/x-www-form-urlencoded</code>，则发送到服务器的HTTP消息正文本质上是一个查询字符串。 键和值在键值对中进行编码，分隔方式为 <code>&amp;</code> 和 <code>=</code> 键和值之间。 </p> <p>对于二进制数据，使用 <code>[!UICONTROL multipart/form-data]</code> 而是。</p> 
...
>[!NOTE]
>
>为了确保您的 [!UICONTROL JSON] 有效，则可以使用可用的在线服务之一，例如 [https://jsonlint.com/](https://jsonlint.com/). 您还可以使用 [!UICONTROL JSON] >[!UICONTROL 创建JSON模块] 以动态创建JSON并完成所有必要的转义。
>
