...
title: HTTP &gt;发出请求模块
description: Adobe Workfront Fusion HTTP &gt;请求模块是一个通用模块，它允许您配置HTTP请求并将其提交到服务器。 接收的HTTP响应随后包含在输出包中。
author: Becky
...
# [!UICONTROL HTTP] >[!UICONTROL 请求] 模块

>[!NOTE]
>
>Adobe Workfront Fusion需要 [!DNL Adobe Workfront Fusion] 除 [!DNL Adobe Workfront] 许可证。

的 [!DNL Adobe Workfront Fusion] [!UICONTROL HTTP] > [!UICONTROL 发出请求模块] 是一个通用模块，用于配置HTTP请求并将其提交到服务器。 接收的HTTP响应随后包含在输出包中。
...
有关 [!DNL Adobe Workfront Fusion] 许可证，请参阅 [Adobe Workfront Fusion许可证](../../../workfront-fusion/get-started/license-automation-vs-integration.md)

## [!UICONTROL HTTP] >[!UICONTROL 请求] 模块配置

当您配置 [!UICONTROL HTTP] >[!UICONTROL 请求] 模块， [!DNL Adobe Workfront Fusion] 显示下面列出的字段。 模块中的粗体标题表示必填字段。
  ...
  <tr> 
   ...
   <td> <p>HTTP Body是在HTTP事务消息中发送的数据字节（如果有任何要使用的数据字节，则紧靠标头后）。</p> 
    <ul> 
     ...
     <li> <p><strong>[!UICONTROL Application/x-www-form-urlencoded]</strong> </p> <p>此正文类型是使用 <code>[!UICONTROL application/x-www-form-urlencoded]</code>.</p> <p>对于 <code>application/x-www-form-urlencoded</code>，则发送到服务器的HTTP消息正文本质上是一个查询字符串。 键和值以键值对进行编码，键值对之间用 <code>&amp;</code> 和 <code>=</code> 键和值之间。 </p> <p>对于二进制数据，请使用 <code>[!UICONTROL multipart/form-data]</code> 中。</p> 
...
>[!INFO]
>
>**示例：** 此示例说明如何设置模块以提交 [!UICONTROL POST] 具有JSON有效负载的请求：
>
>![](assets/make-a-request-example-350x522.png)

>[!NOTE]
>
>确保 [!UICONTROL JSON] 有效，则可以使用其中一项可用的在线服务，例如 [https://jsonlint.com/](https://jsonlint.com/). 您还可以使用 [!UICONTROL JSON] >[!UICONTROL 创建JSON模块] 以动态创建JSON并处理所有必需的转义。
>
>将JSON段与表达式和项目直接混合在 [!UICONTROL 请求内容] 不建议使用字段，因为该字段可能会导致无效的JSON。
