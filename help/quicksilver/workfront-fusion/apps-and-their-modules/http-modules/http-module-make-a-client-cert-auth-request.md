...
title: HTTP &gt；生成客户端证书授权请求模块
description: 此 [!DNL Adobe Workfront Fusion] 模块允许您使用HTTP客户端证书授权配置HTTP请求并将其提交到服务器。 接收的HTTP响应随后包含在输出包中。
author: Becky
...
## [!UICONTROL HTTP] >[!UICONTROL 发出客户端证书授权请求] 模块配置

当您配置 [!UICONTROL HTTP] >[!UICONTROL 发出客户端证书授权请求] 模块， [!DNL Adobe Workfront Fusion] 显示下面列出的字段。 模块中的粗体标题表示必填字段。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>选择包含您的客户端证书身份验证凭据的密钥，或单击 <strong>[！UICONTROL添加]</strong> 以将您的凭据添加到新密钥。 </p> <p>注意：您可以添加更多凭据，以便在每个连接之间轻松切换。</p> </td> 
  </tr> 
  <tr> 
