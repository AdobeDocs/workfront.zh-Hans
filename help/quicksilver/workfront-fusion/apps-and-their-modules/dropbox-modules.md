...
title: Dropbox模块
description: 在 [!DNL Adobe Workfront Fusion] 方案，您可以自动执行使用Dropbox的工作流，并将其连接到多个第三方应用程序和服务。这样，您就可以自动执行Dropbox中的文件和文件夹的监视、搜索、检索、列出、创建和编辑等活动。
author: Becky
...
在 [!DNL Adobe Workfront Fusion] 场景，您可以自动执行使用 [!UICONTROL Dropbox]，并将其连接到多个第三方应用程序和服务。这使您能够自动执行各种活动，例如监视、搜索、检索、列出、创建和编辑您的中的文件和文件夹。 [!UICONTROL Dropbox].
...
使用 [!DNL Dropbox] 模块，您必须具有 [!DNL Dropbox] 帐户。

## [!DNL Dropbox] 模块及其字段

配置时 [!DNL Dropbox] 模块， [!DNL Workfront Fusion] 显示下面列出的字段。 除此以外，还有 [!DNL Dropbox] 可能会显示字段，具体取决于应用程序或服务中的访问级别等因素。 模块中的粗体标题表示必填字段。
...
* [创建和编辑模块 [!DNL Dropbox] 文件和文件夹](#modules-for-creating-and-editing-dropbox-files-and-folders)
* [其他模块](#other-modules)
...
<table style="table-layout:auto">
 ...
 <tbody> 
  <tr> 
   <td>[！UICONTROL连接] </td> 
   <td> <p>有关连接 [!DNL Dropbox] 目标帐户 [!DNL Workfront Fusion]，请参见 <a href="../../workfront-fusion/connections/connect-to-fusion-general.md" class="MCXref xref" data-mc-variable-override="">创建连接 [!DNL Adobe Workfront Fusion]  — 基本说明</a></p> </td> 
  </tr> 
  <tr> 
   <td>[！UICONTROL文件夹] </td> 
  ...
  <tr> 
   ...
   <td> <p> 启用此选项还可以监视所选文件夹中的子文件夹以查找修改的文件。</p> </td> 
  </tr> 
  <tr> 
   <td>[！UICONTROL限制] </td> 
...
此搜索模块查找对象中的记录 [!DNL Dropbox] 与指定的搜索查询匹配的用户名称。
...
<table style="table-layout:auto">
 ...
 <tbody> 
  ...
  <tr> 
   <td> <p>文件路径/文件</p> </td> 
   <td> <p style="font-weight: bold;">文件路径</p> <p>输入目标路径或将目标路径映射到文件。</p> <p style="font-weight: bold;">文件</p> <p>从菜单中选择文件。</p> </td> 
  </tr> 
 </tbody> 
</table>
...
该模块会返回文件夹的ID和任何关联字段，以及连接访问的任何自定义字段和值。 您可以将此信息映射到场景中的后续模块。
...
该模块返回文件或文件夹的ID以及任何关联的字段，以及连接访问的任何自定义字段和值。 您可以将此信息映射到场景中的后续模块。
...
<table style="table-layout:auto">
 ...
 <tbody> 
  ...
  <tr> 
   <td>限制 </td> 
   <td> <p>输入或映射您希望模块在每个方案执行周期内列出的最大记录数。</p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td>[！UICONTROL文件夹]</td> 
   <td> <p> 选择您的文件夹 [!DNL Dropbox] 将文件上传到。</p> </td> 
  </tr> 
  <tr> 
   <td> <p>[！UICONTROL源文件]</p> </td> 
   <td> <p>输入或映射要添加到的文件 [!DNL Dropbox] 以上选定的文件夹。</p> <p style="font-weight: bold;">[！UICONTROL文件名]</p> <p>输入或映射文件名，包括文件扩展名。</p> <p style="font-weight: bold;">[！UICONTROL文件数据]</p> <p>输入或映射文件数据(来自前一模块，如[！UICONTROL Google Drive] &gt;[！UICONTROL Get a File)]。</p> <p>注意：上传文件的最大大小为150 MB。</p> </td> 
  </tr> 
  <tr> 
   <td>[！UICONTROL覆盖现有文件]</td> 
   <td> <p> 启用此选项可将现有文件替换为新文件。 如果将此选项保留为禁用，则将重命名上传的文件。</p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>输入或映射要创建新文件夹的路径。</p> <p>注释:   <p>如果您使用 [!DNL Dropbox Business] 帐户（包含组空格），您必须删除斜杠 <code>/</code>，或者不要单击 <strong>[！UICONTROL单击此处]选择文件夹</strong> 以在根目录中创建团队文件夹。</p> <p>如果未删除斜杠，则出现错误 <code>[409] path/malformed_path/..</code> 会返回。</p> </p> </td> 
  </tr> 
  <tr> 
   <td>[！UICONTROL自动重命名]</td> 
   <td> <p> 启用此选项以重命名新文件夹（如果目标位置中已存在同名文件夹）。</p> </td> 
  </tr> 
 </tbody> 
</table>
...
该模块会返回链接的ID和任何关联字段，以及连接访问的任何自定义字段和值。 您可以将此信息映射到场景中的后续模块。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p style="font-weight: bold;">[！UICONTROL文件路径]</p> <p>输入目标路径或将目标路径映射到文件。</p> <p style="font-weight: bold;">[！UICONTROL文件]</p> <p>从菜单中选择文件。</p> </td> 
  </tr> 
  <tr> 
   <td> <p>[！UICONTROL请求的可见性]</p> </td> 
   <td> <p>选择链接是公共链接、团队链接还是密码限制链接。</p> <p>注意：[！UICONTROL仅团队]和[！UICONTROL通过密码访问]选项仅适用于具有以下权限的用户： [!DNL Dropbox Pro] 或更高版本。</p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>为链接收件人设置权限。</p> <p><strong>[！UICONTROL查看器]</strong> 使用该链接的用户可以查看和评论内容。</p> <p><strong>[！UICONTROL编辑器]</strong> 使用该链接的用户可以编辑、查看和评论内容。</p> <p><strong>[！UICONTROL Max]</strong> 使用该链接的用户会获得您可以设置链接的最高访问级别。</p> </td> 
  </tr> 
 </tbody> 
</table>
...
该模块会返回版本的ID和任何关联的字段，以及连接访问的任何自定义字段和值。 您可以将此信息映射到场景中的后续模块。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p><strong>[！UICONTROL文件路径]</strong> </p> <p>输入目标路径或将目标路径映射到文件。</p> <p><strong>[！UICONTROL文件]</strong> </p> <p>从菜单中选择文件。</p> </td> 
  </tr> 
  <tr> 
   <td> <p>[！UICONTROL修订版]</p> </td> 
   <td> <p>输入或映射要还原的修订版的修订号。</p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>为新位置中的文件或文件夹输入新名称。</p> </td> 
  </tr> 
  <tr> 
   <td> <p>[！UICONTROL自动重命名]</p> </td> 
   <td> <p>启用此选项以确保如果存在同名文件或文件夹，则模块通过在文件或文件夹名称后添加([！UICONTROL NUMBER])来重命名新文件或文件夹。 否则，将覆盖目标位置中的文件或文件夹。</p> </td> 
  </tr> 
  <tr> 
   <td> <p>[！UICONTROL允许所有权转让]</p> </td> 
   <td> <p>启用此选项可允许所有者移动，即使这将导致所移动内容的所有权转移。</p> </td> 
  </tr> 
 </tbody> 
</table>
...
通过此操作模块，您可以对 [!DNL Dropbox] API。 这样，您就可以创建一个其他人无法实现的数据流自动化 [!DNL Dropbox] 模块。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>输入相对路径输入相对路径 <code>https://api.dropboxapi.com</code>. 例如， <code>/2/files/list_folder</code></p> <p>注意：有关可用端点的列表，请参阅 <a href="https://www.dropbox.com/developers/documentation/http/documentation">DropboxAPI v2文档</a>.</p> </td> 
  </tr> 
  <tr> 
...
>`}`
>
>搜索匹配项可在模块的输出中找到，位于 [!UICONTROL 捆绑] > [!UICONTROL 正文] >个条目。
>
>在我们的示例中，返回了10张票证：
...
* 上传的文件太大，超过了允许的最大文件大小 [!DNL Dropbox] 计划，或者您已使用所有 [!DNL Dropbox] 帐户的存储配额。 您必须从删除现有文件 [!DNL Dropbox] 帐户或升级计划。
* 先前选定的文件夹（文件将上载到该文件夹）不再存在。 场景将停止，您必须再次选择目标文件夹。

### 无法呈现通过共享链接引用的图像

返回的URL [!UICONTROL Dropbox] >[!UICONTROL 创建共享链接] 不会直接链接到图像，而是链接到 [!DNL Dropbox] 页面。 要强制下载图像，请替换尾随的 `?dl=0` 替换为 `?dl=1`. 要强制呈现图像(例如，在Web浏览器或Facebook Messenger中)，请附加 `&raw=1` 到URL。
...
您也可以复制该区段并将其粘贴到字段中。 Replace `1.url` 使用URL。
