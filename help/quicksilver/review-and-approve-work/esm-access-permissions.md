...
title: Adobe云存储模型的对象权限和访问级别概述
description: Adobe cloud storage权限和访问概述
author: Courtney
...
Adobe云存储是一种基于云的存储解决方案，它用作Adobe企业产品中资产的中央存储存储库。 使用Adobe cloud storage的Workfront环境的对象权限和访问级别行为与使用旧版Workfront文档存储的环境略有不同。

## 访问级别

Workfront访问级别仅适用于Workfront。 Workfront中的项目和文档限制并不总是适用于其他Adobe应用程序。

### 同时使用Adobe云存储和旧版Workfront存储的环境

根据项目是位于Adobe云存储上还是旧版Workfront存储上，文档访问的行为有所不同：
...
您无法修改使用Adobe云存储的项目、项目和项目组合的访问级别文档权限。
