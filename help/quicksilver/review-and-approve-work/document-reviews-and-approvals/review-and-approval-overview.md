...
>[!IMPORTANT]
>
>此功能即将推出。 目前，Frame.io帐户由Workfront团队手动添加。 请联系您的Adobe客户代表。
...
* **审阅者** 可以添加注释和标记资源。 完成后，他们可以将其审阅标记为完成。 资产无需在审批流程中继续前进，即可将审核标记为完成。<!--example of when to add reviewers-->
...
* **审批模板**：在Workfront设置区域，项目协调员可以创建可恢复的审批模板。 在模板中，用户可以添加审阅人和批准者，并指定完成时间范围。 将审批模板应用于资产时，截止日期从指定的时间范围中计算。
  ...
  有关更多信息，请参阅 [创建和管理审批模板]
  <!--add link.-->

* 从Workfront上传资源并将其发送到框架以供审阅和批准 <!-- delayed? -Need more info on this workflow -->

### 批准Frame.io中的资源

利益相关者可以使用Frame.io查看器查看和批准连接的资源。

有关在Frame.io中查看资产的更多信息，请参阅 <!-- get link from frame team?-->
...
* 批准：资产无需更改，随时可使用。
* 批准，但有更改：资产需要更改，并且可在做出更改后使用。 不需要额外审批。
* 需要工作：资产需要更改，但还未准备就绪。 完成指定的更改后，必须将资源上传为新版本，并经过另一轮批准。 <!--is the same approval workflow automatically applied? Does the coordinator have to do anything to get the approval going? -->

审阅人可以在Frame.io审阅人中将其审阅标记为完成，但资产在审批过程中无需进行此标记。 <!--check if this is accurate -->
...
### 将批准的资源发送到Adobe Experience Manager Assets

<!-- deprioritized?-->
使用本机集成将批准的资源发送到Adobe Experience Manager Assets。
...
![](assets/example-workflow.png)  — 可能需要此视频的不同版本，甚至需要视频演练
